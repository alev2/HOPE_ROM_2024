function [Params]=InitializeParameters(ExcelFileName)
...
%               350, CD4<200) 
%                   equivalent to: Acute, Latent, Late, AIDS
...
%% 2. Define variables that describe model structure
    ...
    Params.A7=2; % A7 Sus, on-oral-PrEP, high adherence
    Params.A8=3; % A8 Sus, on-oral-PrEP, low adherence JCPrEPUpdate: new PrEP states starting here - other compartment nums shifted by 3
    Params.A9=4; % A9 Sus, on-injectable-PrEP, high adherence
    Params.A10=5; % A10 Sus, on-injectable-PrEP, low adherence
    Params.B1=6; % B1 Acute UA
    Params.B2=7; % B2 Acute Aware
    Params.B3=8; % B3 Acute In Care
    Params.C1=9; % C1 CD4 350 UA
    Params.C2=10; % C2 CD4 350 Aware
    Params.C3=11; % C3 CD4 350 In Care
    Params.C4=12; % C4 CD4 350 ART-not-VLS
    Params.C5=13; % C5 CD4 350 oral ART VLS
    Params.C6=14; % C6 CD4 350 LAI ART VLS
    Params.D1=15; % D1 CD4 200 UA
    Params.D2=16; % D2 CD4 200 Aware
    Params.D3=17; % D3 CD4 200 In Care
    Params.D4=18; % D4 CD4 200 ART-not-VLS
    Params.D5=19; % D5 CD4 200 oral ART VLS
    Params.D6=20; % D6 CD4 200 LAI ART VLS
    Params.E1=21; % E1 AIDS UA
    Params.E2=22; % E2 AIDS Aware
    Params.E3=23; % E3 AIDS In Care
    Params.E4=24; % E4 AIDS ART-not-VLS
    Params.E5=25; % E5 AIDS Oral ART VLS
    Params.E6=26; % E6 AIDS LAI ART LVS
    Params.Compart_NormDeath =27; % Normal Death
    Params.Compart_AIDSDeath =28; % AIDS/ART Death

        
% TT CONTINUUM COMPARTMENTS

    % Unaware
    UnawareComparts = [Params.B1, Params.C1,Params.D1, Params.E1];
    
    % Aware
    AwareComparts = [Params.B2:Params.B3,Params.C2:Params.C6,Params.D2...
        :Params.D6,Params.E2:Params.E6];
    % In Care
    CCStages3456 = [Params.B3,Params.C3:Params.C6,Params.D3...
        :Params.D6,Params.E3:Params.E6];
    
    % ART only
    ANVComparts = [Params.C4, Params.D4,Params.E4]; 
        
    % ART and VLS
   ARTandVLSComparts = [Params.C4:Params.C6,Params.D4:Params.D6,...
       Params.E4:Params.E6]; 
    % Oral ART VLS
    CCStage5 = [Params.C5, Params.D5, Params.E5];
    % LAI ART VLS
    CCStage6 = [Params.C6, Params.D6, Params.E6];
    % VLS
   VLSComparts = [Params.C5:Params.C6, Params.D5:Params.D6, Params.E5:Params.E6];
   % PrEP
   PrEPComparts = Params.A7:Params.A10;
   % Oral PrEP
   OralPrEPComparts = Params.A7:Params.A8;
   % Injectable PrEP
   InjectPrEPComparts = Params.A9:Params.A10;
        
    % Apply to Params
    Params.UnawareComparts = UnawareComparts;
    Params.AwareComparts = AwareComparts;
    Params.CCStages3456 = CCStages3456;
    Params.ANVComparts = ANVComparts;
    Params.ANVandVLSComparts = ARTandVLSComparts;
    Params.OralARTVLSComparts = CCStage5;
    Params.LAIARTVLSComparts = CCStage6;
    ...
    % DISEASE PROGRESSION COMPARTMENTS % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
    ...
    LatentComparts = Params.C1:Params.C6;
    LateComparts = Params.D1:Params.D6;
    AIDSComparts = Params.E1:Params.E6;
    HIVComparts = Params.B1:Params.E6;
    
    Params.NonAbsorbingComparts = Params.A1:Params.E6;
    ...
    Params.LatentComparts = LatentComparts;
    ...
    for stratificationsSection = 1:1
        Params.numStrats = 168; % all subpopulations
        ...
        Params.numIntnCostLvls = 4; %Used for intervention costs that vary by reach (Std, Low, Med, Intensive)
        Params.numAge = 7;
        numPop = 3; % HET, MSM, IDU
        numPopandRiskLevelGroups_Female = 3; % low-risk HET, high-risk HET, IDU
        numPopandRiskLevelGroups_Male = 5; % low-risk HET, high-risk HET, low-risk MSM, high-risk MSM, IDU
        ...
        numHIVstages = 4; %Revised by KH to collapse 350-500 and >500
        Params.numHIVstages = numHIVstages;
        numChronicDiseaseStages = 3; %Revised by KH to collapse 350-500 and >500
        numDistinctContStagesAcute = 3; % Unaware,Aware,LTC
        numDistinctContStagesWithCosts = 5; % See below for notes:
            % Based on the "annual care and treatment costs input in Excel"
            % The 5 refers to unaware, aware, LTC, oral ART, on LAI ART - edited by LX on 8/9/2024 to adjust for row 6
 
        Params.numModelsContStagesIfChronic = 6; % Unaware, Aware no ART, LTC no ART, ANV, VLS, LAI-VLS - edited by LX on 8/9/2024 to adjust for row 6
        ...
        Params.numIntnIncrCostCategories = 9; % Increased from 8 to 9 by KH on 27Dec2023
        ...
        % Number of categories of testing costs
        Params.numTestCostCategories = 4; % 1:NegRapid, 2:PosRapid, 3:NegConv, 4:PosConv
    end
...
% INDICATOR: RACE/ETHNICITY
    numSubPopsinRace = Params.numAge * (numPopandRiskLevelGroups_Female + numPopandRiskLevelGroups_Male); 

    % Fill indicator
        Params.raceIndicator(1:numSubPopsinRace,1) = 1; % Black
        Params.raceIndicator((numSubPopsinRace + 1):(numSubPopsinRace * 2),2) = 1; % Hispanic
        Params.raceIndicator((numSubPopsinRace * 2 + 1):(numSubPopsinRace * 3),3) = 1; % Other
        raceIndicator = zeros(Params.numStrats,numRace);
        raceIndicator(1:56,1) = 1; % Black
        raceIndicator(57:112,2) = 1; % Hispanic
        raceIndicator(113:168,3) = 1; % Other
...
% INDICATOR: TRANSMISSION GROUP AKA POPULATION
    ...
    % Fill indicator
        %HET Column
            SubpopsBetweenHET_Male = Params.numAge * (numPopandRiskLevelGroups_Male - 2); % 2 subtracted out because there are 2 pop/risk-level groups for HET males (low-risk HET, high-risk HET)
            SubpopsBetweenHET_Female = Params.numAge * (numPopandRiskLevelGroups_Female - 2); % 2 subtracted out because there are 2 pop/risk-level groups for HET females (low-risk HET, high-risk HET)
            numHETSubpopsperLoop = numRiskLevel * Params.numAge; % number of HET subpopulations to populate with 1's in indicator each loop
            numLoops = Params.numRace * numSex; % number of HET subgroups to populate with 1's in the indicator 
            storeHET = repmat([21;7],[3,1]); % This var tells the loop below how many rows to skip to get to the next HET group
            count = 1;
            for a = 1:6
            ...
            storeHETnew = repmat([SubpopsBetweenHET_Male;SubpopsBetweenHET_Female],[Params.numRace,1]);
            count = 1;
            for a = 1:numLoops
                popIndicatorNew(count:count+numHETSubpopsperLoop-1,Params.pop_HET)=1;
                count = count+storeHETnew(a)+numHETSubpopsperLoop;
            end
        %MSM Column
            MSMStart = Params.numAge * numRiskLevel; % First MSM subpopulation starts after LR HETM and HR HETM
            SubpopsBetweenMSM = Params.numAge * (numSex + (numSex * numRiskLevel)); % PWIDM, PWIDF, HETF, and HETM between MSM groups 
            numMSMSubpopsperLoop = Params.numAge * numRiskLevel; % Each MSM group is by age and risk level
            storeMSM = [42;42;1];
            count = 15;
            for a = 1:3
...
% INDICATOR: SEX
    ...
    % Fill indicator
        ...
        count = 1;
        for a = 1:3
            Params.sexIndicator(count:count+34,1)=1;
            count = count+21+35;
        end
...
% INDICATOR: RISK LEVEL
    ...
    % Fill indicator
        % Main
        storeMain= [7;14;14;7;14;14;7;14;1];
        count=1;
        for a = 1:9
...
% DISEASE STAGES % updated by KH on 14Jun2024 to collapse 350-500 and >500
    Params.stage_Acute = 1; % B
    Params.stage_Latent = 2; % C
    Params.stage_Late = 3; % D
    Params.stage_AIDS = 4; % E
...
% Model's Continuum of care stages if chronic HIV
    ...
    Params.chronicContStage_LAIARTVLS = 6;
...
% Distinct Continuum stages
    ...
    Params.distinctContStafe_LAIART = 5;
...
% INDICATOR: AWARE COMPARTMENTS
    ...
    % [Stratsx Compartments]
    
    inf_awareComparts_StratsxStratsxComparts=zeros(Params.numStrats,Params.numStrats,Params.numComparts);
    inf_awareComparts_StratsxStratsxComparts(:,:,AwareComparts)=1;
...
%% 4. Define Model Settings
    ...
    % Apply differentiated spending amounts for transitions along the ...
    % continuum of care and engagement in prevention interventions ...
    % (i.e., more spending required for increased levels of reach)?
    % This on/off switch is functionalized in Excel and is only pulled into
    % MATLAB for record-keeping purposes..
    % 0 = no
    % 1 = yes

    Params.ApplyDiffCostsByReach = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;
   ...
   if applyIntervalAllHET == 1
    ...
    % Period 1 relative risks
        ...
        % Relative risk by disease stage
            % Acute, Latent, Late, AIDS, Uninfected
                %Note: reference case is Latent. It is not the first element
                %so that it remains consistent with the order of the
                %previously-defined indicators (e.g., Params.stage_Latent = 2)         
        % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
    ...
    Params.tt_testRateLatent_rp_1(Params.numStrats)=1;
    ...
    Params.tt_testRateLatent_rp_2to4(Params.numStrats)=1;
    ...
    Params.tt_testRateLatent_rp_5(Params.numStrats)=1;
    ...
    for s = 1:(numHIVstages+1)
        ...
        switch s
           case 1
                ...
                testingStore = zeros(Params.numStrats,1);
           case 2
                Params.tt_testRateLatent_rp_1 = testingStore;
                clear testingStore 
                testingStore = zeros(Params.numStrats,1);
           case 3
    ...
    for s = 1:(numHIVstages+1)
       ...
       switch s
           case 1
                ...
                testingStore = zeros(Params.numStrats,1);
           case 2
                Params.tt_testRateLatent_rp_2to4 = testingStore;
                clear testingStore 
                testingStore = zeros(Params.numStrats,1);
           case 3
        ...
        for s = 1:(numHIVstages+1)
           ...
           switch s
               ...
               case 2
                    switch loop
                        case 1
                            Params.tt_testRateLatent_rp_5 = testingStore;
                        case 2
                            Params.tt_testRateLatent_rp_5_nonYMSM = testingStore .* Params.nonYMSMIndicator;
                    end
                    clear testingStore 
                    testingStore = zeros(Params.numStrats,1);
               case 3
...
% TEST SENSITIVITIES
    ...
    % Period 1
        ...
        for nHIVstage = Params.stage_Acute:Params.stage_AIDS
            ...
            switch nHIVstage
                ...
                case Params.stage_Latent
                    Params.tt_linkageAfterRate_Latent_1 = calculatedVariable;
    ...
    % Period 2 to 4
        ...
        for nHIVstage = Params.stage_Acute:Params.stage_AIDS
            ...
            switch nHIVstage
                ...
                case Params.stage_Latent
                    Params.tt_linkageAfterRate_Latent_2to4 = calculatedVariable;
   ...
   % if running allocation-based progression and targeting to YMSM only,
        ...
        for nHIVstage = Params.stage_Acute:Params.stage_AIDS
            ...
            switch nHIVstage
                ...
                case Params.stage_Latent
                    switch loop
                        case 1
                            Params.tt_linkageAfterRate_Latent_5 = calculatedVariable;
                        case 2
                            Params.tt_linkageAfterRate_Latent_5_nonYMSM = calculatedVariable.* Params.nonYMSMIndicator;
                    end
    ...
    % Dropping from [In Care] to [Aware]
        ...
        % Apply race-specific values to full Stratsx1 subpopulation variables            
   ...
   % Dropping from [ART-not-VLS] to [Aware]
    ...
    % Dropping from [ART-not-VLS] to [In Care]
        ...
        % Read values in from Excel
         ...
         % Apply relative risks to [Stratsx1] matrix
 ...
 % Dropping from [LAI-ART-VLS] to [LTC] % Added by Lola 08/07
        % Read in probabilities from Excel
        store_probDropOutLAIVLSToLTC(numRace,Params.numRateInputPeriod) = 0; 
        ...
        for k = 1:Params.numRateInputPeriod
            store_probDropOutLAIVLSToLTC(:, k) = ExcelValues_AllParameters((parameter_Index+numRace*(k-1)): ...
                ((parameter_Index + numRace-1)+numRace*(k-1)));
        end
        parameter_Index = parameter_Index + numel(store_probDropOutLAIVLSToLTC);


        % Convert to rates
        store_rateDropOutLAIVLSToLTC = -log(1-store_probDropOutLAIVLSToLTC);
        
        % Apply race-specific values to full Stratsx1 subpopulation variables            
        tt_dropOutRate_LAIVLSToLTC_1 = Params.raceIndicator*store_rateDropOutLAIVLSToLTC(:,Params.period_1);
        tt_dropOutRate_LAIVLSToLTC_2to4 = Params.raceIndicator*store_rateDropOutLAIVLSToLTC(:,Params.period_2to4);
        tt_dropOutRate_LAIVLSToLTC_5 = Params.raceIndicator*store_rateDropOutLAIVLSToLTC(:,Params.period_5);


        % Read in relative risks of dropping off LAI ART by transmission
        % Read values in from Excel
            % Period 1 [3x1]    
            Params.store_relRiskDropOutLAIVLSToLTCPop_1(:,1) = ...
                ExcelValues_AllParameters(parameter_Index :(parameter_Index + numPop - 1));
                parameter_Index = parameter_Index + numPop;
            
            % Period 2 to 4 [3x1]    
            Params.store_relRiskDropOutLAIVLSToLTCPop_2to4(:,1) = ...
                ExcelValues_AllParameters(parameter_Index :(parameter_Index + numPop - 1));
                parameter_Index = parameter_Index + numPop;
            
            % Period 5 [3x1]    
            Params.store_relRiskDropOutLAIVLSToLTCPop_5(:,1) = ...
                ExcelValues_AllParameters(parameter_Index :(parameter_Index + numPop - 1));
                parameter_Index = parameter_Index + numPop;
    
         % Apply relative risks to [Stratsx1] matrix
            tt_relRiskDropOutLAIVLSToLTCPop_1 = Params.popIndicator * Params.store_relRiskDropOutLAIVLSToLTCPop_1;
            tt_relRiskDropOutLAIVLSToLTCPop_2to4 = Params.popIndicator * Params.store_relRiskDropOutLAIVLSToLTCPop_2to4;
            tt_relRiskDropOutLAIVLSToLTCPop_5 = Params.popIndicator * Params.store_relRiskDropOutLAIVLSToLTCPop_5;
        
        % Read in relative risks of dropping off LAI ART by age group
        % Read values in from Excel
            % Period 1 [7x1]    
            Params.store_relRiskDropOutLAIVLSToLTCAge_1(:,1) = ...
                ExcelValues_AllParameters(parameter_Index :(parameter_Index + Params.numAge - 1));
                parameter_Index = parameter_Index + Params.numAge;
            
            % Period 2 to 4 [7x1]    
             Params.store_relRiskDropOutLAIVLSToLTCAge_2to4(:,1) = ...
                ExcelValues_AllParameters(parameter_Index :(parameter_Index + Params.numAge - 1));
                parameter_Index = parameter_Index + Params.numAge;
            
            % Period 5 [7x1]    
             Params.store_relRiskDropOutLAIVLSToLTCAge_5(:,1) = ...
                ExcelValues_AllParameters(parameter_Index :(parameter_Index + Params.numAge - 1));
                parameter_Index = parameter_Index + Params.numAge;
    
         % Apply relative risks to [Stratsx1] matrix
            tt_relRiskDropOutLAIVLSToLTCAge_1 = Params.ageIndicator * Params.store_relRiskDropOutLAIVLSToLTCAge_1;
            tt_relRiskDropOutLAIVLSToLTCAge_2to4 = Params.ageIndicator * Params.store_relRiskDropOutLAIVLSToLTCAge_2to4;
            tt_relRiskDropOutLAIVLSToLTCAge_5 = Params.ageIndicator * Params.store_relRiskDropOutLAIVLSToLTCAge_5;
        
        % Apply relative risks to LAI-VLS drop-out rates    
        Params.tt_dropOutRate_LAIVLSToLTC_1 = tt_dropOutRate_LAIVLSToLTC_1 .* tt_relRiskDropOutLAIVLSToLTCPop_1.* tt_relRiskDropOutLAIVLSToLTCAge_1;
        Params.tt_dropOutRate_LAIVLSToLTC_2to4 = tt_dropOutRate_LAIVLSToLTC_2to4 .* tt_relRiskDropOutLAIVLSToLTCPop_2to4.* tt_relRiskDropOutLAIVLSToLTCAge_2to4;
        Params.tt_dropOutRate_LAIVLSToLTC_5 = tt_dropOutRate_LAIVLSToLTC_5 .* tt_relRiskDropOutLAIVLSToLTCPop_5.* tt_relRiskDropOutLAIVLSToLTCAge_5;   
...
% ART PRESCRIPTION, NOT ART-EFFECTED
    ...
    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
        ...
        % Adjust for eligibility
            ...
            base_initARTProb_1 = store_initARTProbGivenElig_1;
            base_initARTProb_2to4 = store_initARTProbGivenElig_2to4;
            base_initARTProb_5 = store_initARTProbGivenElig_5;
            base_initARTProb_5_nonYMSM = store_initARTProbGivenElig_5_nonYMSM;
    ...
    % Relative risk of ART Initiation by race 
     ...
     % ART initiation rates by disease stage and race [Stratsx1]
               
        % Period 1
            % Loop through HIV stages
            for nHIVstage = Params.stage_Acute:Params.stage_AIDS
                
                % Calculation: [1 base rate] * [Stratsx1 of relative risks]
                ...
                switch nHIVstage
                    ...
                    case Params.stage_Latent
                        Params.tt_ARTInitRateLatent_1 = calculatedVariable;
        ...
        % Period 2 to 4
            ...
            for nHIVstage = Params.stage_Acute:Params.stage_AIDS
                ...
                switch nHIVstage
                    ...
                    case Params.stage_Latent
                        Params.tt_ARTInitRateLatent_2to4 = calculatedVariable;
            ...
            for nHIVstage = Params.stage_Acute:Params.stage_AIDS
                ...
                switch nHIVstage
                    ...
                    case Params.stage_Latent
                        switch loop
                            case 1
                                Params.tt_ARTInitRateLatent_5 = calculatedVariable;
                            case 2
                                Params.tt_ARTInitRateLatent_5_nonYMSM = calculatedVariable .* Params.nonYMSMIndicator;
                        end    
    ...
    % Percentage of ART initiators on LAI ART (versus oral) ART, by race/ethnicity and time period 
        %Added by LX, 8/9/2024
        tt_pctInitLAI_ry(numRace,Params.numRateInputPeriod) = 0;

        for k=1:Params.numRateInputPeriod
            tt_pctInitLAI_ry(:, k) = ExcelValues_AllParameters((parameter_Index+numRace*(k-1)): ...
                ((parameter_Index + numRace-1)+numRace*(k-1)));
        end

        parameter_Index = parameter_Index + numel(tt_pctInitLAI_ry);

        %Preallocate
        Params.tt_pctInitLAI_1(Params.numStrats,1)= 0;
        Params.tt_pctInitLAI_2to4(Params.numStrats,1)= 0;
        Params.tt_pctInitLAI_5(Params.numStrats,1)= 0;

        %Restratify
        Params.tt_pctInitLAI_1 = Params.raceIndicator*tt_pctInitLAI_ry(:,Params.period_1);
        Params.tt_pctInitLAI_2to4 = Params.raceIndicator*tt_pctInitLAI_ry(:,Params.period_2to4);
        Params.tt_pctInitLAI_5 = Params.raceIndicator*tt_pctInitLAI_ry(:,Params.period_5);
...
% Annual probability of switching to LAI ART from oral ART if not VLS
% Added by LX, 08/09/2024

store_probOralANVtoLAI(numRace,Params.numRateInputPeriod) = 0; 
        ...
        for k = 1:Params.numRateInputPeriod
            store_probOralANVtoLAI(:, k) = ExcelValues_AllParameters((parameter_Index+numRace*(k-1)): ...
                ((parameter_Index + numRace-1)+numRace*(k-1)));
        end
        parameter_Index = parameter_Index + numel(store_probOralANVtoLAI);

        % Convert to rates
        store_rateOralANVtoLAI = -log(1-store_probOralANVtoLAI);
        
        % Apply race-specific values to full Stratsx1 subpopulation variables            
        Params.tt_OralANVtoLAI_1 = Params.raceIndicator*store_rateOralANVtoLAI(:,Params.period_1);
        Params.tt_OralANVtoLAI_2to4 = Params.raceIndicator*store_rateOralANVtoLAI(:,Params.period_2to4);
        Params.tt_OralANVtoLAI_5 = Params.raceIndicator*store_rateOralANVtoLAI(:,Params.period_5);


% Annual probability of switching to LAI ART from oral ART if VLS
% Added by LX, 08/09/2024

store_probOralVLStoLAI(numRace,Params.numRateInputPeriod) = 0; 
        ...
        for k = 1:Params.numRateInputPeriod
            store_probOralVLStoLAI(:, k) = ExcelValues_AllParameters((parameter_Index+numRace*(k-1)): ...
                ((parameter_Index + numRace-1)+numRace*(k-1)));
        end
        parameter_Index = parameter_Index + numel(store_probOralVLStoLAI);

        % Convert to rates
        store_rateOralVLStoLAI = -log(1-store_probOralVLStoLAI);
        
        % Apply race-specific values to full Stratsx1 subpopulation variables            
        Params.tt_OralVLStoLAI_1 = Params.raceIndicator*store_rateOralVLStoLAI(:,Params.period_1);
        Params.tt_OralVLStoLAI_2to4 = Params.raceIndicator*store_rateOralVLStoLAI(:,Params.period_2to4);
        Params.tt_OralVLStoLAI_5 = Params.raceIndicator*store_rateOralVLStoLAI(:,Params.period_5);



% Annual probability of switching to oral ART, VLS from LAI ART 
% Added by LX, 08/09/2024

store_probLAItoOralVLS(numRace,Params.numRateInputPeriod) = 0; 
        ...
        for k = 1:Params.numRateInputPeriod
            store_probLAItoOralVLS(:, k) = ExcelValues_AllParameters((parameter_Index+numRace*(k-1)): ...
                ((parameter_Index + numRace-1)+numRace*(k-1)));
        end
        parameter_Index = parameter_Index + numel(store_probLAItoOralVLS);

        % Convert to rates
        store_rateLAItoOralVLS = -log(1-store_probLAItoOralVLS);
        
        % Apply race-specific values to full Stratsx1 subpopulation variables            
        Params.tt_LAItoOralVLS_1 = Params.raceIndicator*store_rateLAItoOralVLS(:,Params.period_1);
        Params.tt_LAItoOralVLS_2to4 = Params.raceIndicator*store_rateLAItoOralVLS(:,Params.period_2to4);
        Params.tt_LAItoOralVLS_5 = Params.raceIndicator*store_rateLAItoOralVLS(:,Params.period_5);
...
% PERCENTAGE OF HIGH-RISK THAT ARE PREP-ELIGIBLE
    ...
    % Apply to full Param [Stratsx1], only high-risk
...
% PrEP INITIATION RATES %JCPrEPUpdate: Updated to read in HET rates by sex
    ...
    % Apply to [Stratsx1]
        % Steps
            ...
            % Apply to full [Stratsx1] variable
...
% COVID Inputs - Added by MClinkscales 6/2/2022; JC updated on 8/29/2022
    ...
    % Effects on annual testing rates: uninfected people
        ...
        % Apply to #Strats subpopulations using race indicator (results in Strats
    ...
    % Effects on annual testing rates: undiagnosed PWH
        ...
        % Apply to #Strats subpopulations using race indicator (results in Strats x 3)
    ...
    % Effects on backwards continuum progression        
        ...
        % Apply to #Strats subpopulations using age indicator (results in Strats x
    ...
    % Effects on sexual contacts
        ...
        % Apply to #Strats subpopulations using pop indicator (results in Strats x
        % 3) and turn on/off effect based on model settings
...
% MSMHigh = Params.ageIndicator * store_MSMHigh_Black ...
    ...
    % Convert subgroups to a single Parameter [Stratsx1]
    %adding age stratification. Laurel 4/28/17
    baseSexContacts = ...
...
% ANNUAL NUMBER OF SEXUAL CONTACTS (ADJUSTED)
    ...
    for Period = 1:(Params.numPeriod - 1)
        ...
        % Apply to full matrix [StratsxStratsx Compartments]
...
% PERCENT OF CONTACTS PROTECTED WITH A CONDOM
    ...
    %VI
        ...
        for p=1:numPop
            ...
            % [Stratsx1] VI condom use for population p
            ...
            % Add to single parameter [Stratsx1]
    ...
    % AI
        ...
        % [StratsxStrats] Male-Female and Female-Male AI condom use
        ...
        % [StratsxStrats] Male-Male AI condom use
        ...
        % Sum values for HET and MSM-MSM partnerships [StratsxStrats]
        store_basePctCondomA = storeHET_basePctCondomA + storeMSM_basePctCondomA;

        % Expand to [StratsxStratsx Compartments]
...
% PERCENT OF CONTACTS PROTECTED WITH A CONDOM (ADJUSTED)
    ...
    % VI Contacts
        ...
        % Expand base number of contacts to be [StratsxStratsx Compartments]
     ...
     % AI Contacts
        ...
        store_factor_adjPctCondomUseA_NoPrEP_1to4 = behav_basePctCondomA .* ...
            (1 + inf_awareComparts_StratsxStratsxComparts * behav_incrCondomUseDueToDiag_1to4);
        store_factor_adjPctCondomUseA_NoPrEP_5 = behav_basePctCondomA .* ...
            (1 + inf_awareComparts_StratsxStratsxComparts * behav_incrCondomUseDueToDiag_5);
...
% ANNUAL NUMBER OF SEXUAL PARTNERS 
    ...
    % to have a single dimension of #Strats cohorts).
...
% SEXUAL MIXING: TRANSMISSION GROUP AND SEX    
    ...
    % Apply to storage parameters [StratsxStrats]
    
        % Transmission group/sex combinations [StratsxStrats]
...
% Adapted by KH on 20Sep2023 to accomodate mixing that varies for HR and LR HETs        
    ...
    % Create full [StratsxStrats] matrix of each trans group/sex sexual combo
...
% NEEDLE MIXING: TRANSMISSION GROUP AND SEX            
    ...
    % Create full [StratsxStrats] matrix with each needle combo
...
% SEXUAL MIXING: OTHER FACTORS
    ...
    % RISK LEVEL
        ...
        % Define indicators
            ...
            % [Stratsx2]: dim 1 is low risk, dim 2 is high risk
        ...
        % Apply to full [StratsxStrats] matrix        
...
% NEEDLE MIXING (OTHER FACTORS)
    ...
    % Needle mixing by race/eth
        ...
        %Restratify store_mix_race_N from 3x3 to StratsxStrats
...
% CALCULATE FULL NEEDLE MIXING MATRIX
    ...
    Params.behav_NeedleMixing =  behav_needleMix_popSex .* ...
...
% PERCENTAGE OF PEOPLE WHO HAVE AI IN THEIR MALE-FEMALE PARTNERSHIPS     
    ...
    for RatePeriod = 1:Params.numRateInputPeriod
        ...
        % Preallocate
        AIPrev_Overall_Strats = zeros(Params.numStrats,1);
        ...
        % Apply the age- and sex- specific AI rates to the full #Strats
        % parameter. Zero out cohorts which are not applicable to the
        % current race.
            % Moving from [10x1] -> [Stratsx1]
        AIPrev_raceSpecific_Strats = (Params.ageIndicator * ...
        ...
        % Apply the race specific AI parameter to the all race parameter.
            ...
            % overallStrats parameter
        AIPrev_Overall_Strats = AIPrev_Overall_Strats + AIPrev_raceSpecific_Strats;
        ...
        switch RatePeriod
            case 1
                Params.pctPeopleA_InMF_1 = AIPrev_Overall_Strats;
            case 2
                Params.pctPeopleA_InMF_2to4 = AIPrev_Overall_Strats;
            case 3
                Params.pctPeopleA_InMF_5 = AIPrev_Overall_Strats;
        end
        
        clear AIPrev_OverallStrats
...
% PERCENTAGE OF PARTNERSHIPS THAT INCLUDE AI (modified 07/31/2018)
    ...
    % Re-arrange matrix for Params struct to be used in CalcInfectionRates (Compartments xStratsxStrats)
...
% COMPARTMENT COSTS
    ...
    % Goal is to apply the correct costs to each compartment [Compartments x1]
    ...
    % Apply to compartment-compatible matrix
        ...
        % Eg. 
                %                          Latent      Late    AIDS
                %   Unaware                $X          $X      $X
                %   Aware, no ART effects  $X          $X      $X
                %   LTC, no ART effects    $X          $X      $X                 
                %   ANV                    $X          $X      $X
                %   VLS                    $X          $X      $X
         ...
         % 6th row (LAI-VLS)- added by LX on 8/9/2024 to adjust for row 6
         costsByModelsContStageChronic_1(Params.chronicContStage_LAIARTVLS,:) = store_ChronicTreatCostsByDistinctContStage(Params.distinctContStafe_LAIART,:);
         costsByModelsContStageChronic_2to4(Params.chronicContStage_LAIARTVLS,:) = store_ChronicTreatCostsByDistinctContStage(Params.distinctContStafe_LAIART,:);
         costsByModelsContStageChronic_5(Params.chronicContStage_LAIARTVLS,:) = store_ChronicTreatCostsByDistinctContStage(Params.distinctContStafe_LAIART,:);
    ...
    % PrEP costs %JCPrEPUpdate: updated to read in PrEP cost for each state
        ...
        % Acute
            Params.hiv_annualCostPerCompart_1(AcuteComparts) = costsByDistinctContStageAcute;
            Params.hiv_annualCostPerCompart_2to4(AcuteComparts) = costsByDistinctContStageAcute;
            Params.hiv_annualCostPerCompart_5(AcuteComparts) = costsByDistinctContStageAcute;
                
        % Chronic
            Params.hiv_annualCostPerCompart_1(Params.C1:Params.E6) = costsByModelsContStageChronic_1;
            Params.hiv_annualCostPerCompart_2to4(Params.C1:Params.E6) = costsByModelsContStageChronic_2to4; 
            Params.hiv_annualCostPerCompart_5(Params.C1:Params.E6) = costsByModelsContStageChronic_5; 
...
% HIV CONTINUUM OF CARE TRANSITION COSTS (added by JC on 02/07/2018; commented out by KH on 12/15/2023 to replace with intervention costs)            
...
% NUMBER OF YEARS IN EACH AGE GROUP
    ...
    % Apply to subpopulation matrix [Stratsx1]
...
% NUMBER OF YEARS IN EACH DISEASE STAGE (modified by JC on 08/02/2018)
    ...
    % Natural history [5xStrats] 
        % Applied to all PLWH who are not [ART-not-VLS] or [VLS]
        % Acute, CD4>350, CD4 200-350, CD4<200
        % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
    ...
    % ART-not-VLS
        ...
        hiv_durHIVStage_ANV_Latent_Age(1,:) = ExcelValues_AllParameters(parameter_Index: ...
                parameter_Index + numAgeGroups - 1);
        parameter_Index = parameter_Index + numAgeGroups;
        
        hiv_durHIVStage_ANV_Latent(1,1:4) = hiv_durHIVStage_ANV_Latent_Age(1,1);
        hiv_durHIVStage_ANV_Latent(1,5:Params.numAge-1) = hiv_durHIVStage_ANV_Latent_Age(1,2);
        hiv_durHIVStage_ANV_Latent(1,Params.numAge) = hiv_durHIVStage_ANV_Latent_Age(1,3);
        ...
        Params.hiv_durHIVStage_ANV_Latent = hiv_durHIVStage_ANV_Latent * Params.ageIndicator';
...
% DISEASE STAGE TRANSITION RATES WHILE VLS (modified on 08/01/2018 - JC to modify again)
    ...
    % Decrease in CD4 count 
        ...
        % Read in from Excel
        Params.hiv_rateVLSCD4decr_Latent = ExcelValues_AllParameters(parameter_Index);
        parameter_Index = parameter_Index + 1;
  ...
  %Preallocate
    ...
    for s = 1:4 
        ...
        storeAllAge = zeros(Params.numStrats,1);
            ...
            ageSpecific_Strats = Params.ageIndicator * ageDiseaseSpecificRateDeath; 

            storeAllAge = storeAllAge + ageSpecific_Strats;
...
% OVERALL SEXUAL TRANSMISSION PROBABILITIES (BY CONTACT TYPE)
    % Per unprotected contact between HIV+ and HIV- partners
...
% BASE TRANSMISSION RATE
    ...
    % Vaginal Insertive
        ...
        inf_basePerActProb_VaginalIns(:,:,LatentComparts)=inf_overallProb_VaginalIns * Params.store_relRiskSexTransmission(Params.stage_Latent);
    ...
    % Anal Insertive
        ...
        inf_basePerActProb_AnalIns(:,:,LatentComparts)=inf_overallProb_AnalIns * Params.store_relRiskSexTransmission(Params.stage_Latent);
   ...
   % Vaginal receptive
        ...
        inf_basePerActProb_VaginalRec(:,:,LatentComparts)=inf_overallProb_VaginalRec * Params.store_relRiskSexTransmission(Params.stage_Latent);
   ...
   % Anal receptive
        ...
        inf_basePerActProb_AnalRec(:,:,LatentComparts)=inf_overallProb_AnalRec * Params.store_relRiskSexTransmission(Params.stage_Latent);
...
% NEEDLE TRANSMISSION PROBABILITY
    ...
    % Apply to IDU-IDU combinations, by disease stage
        ...
        % Only allow IDU-IDU combinations
            ...
            for c = Params.B1:Params.E6
                inf_indicator_IDU_IDU(:,:,c) = Params.idx_IDU_IDU;
            end

        % Adjust from StratsxStratsx Compartments to Compartments xStratsxStrats
        ...
        % matrix [Compartments xStratsxStrats]
        ...
        inf_basePerNeedleTransProb(LatentComparts,:,:) = inf_indicator_IDU_IDU(LatentComparts,:,:)*store_needleTransmissionProb(Params.stage_Latent);
...
% TRANSMISSION REDUCTION DUE TO VLS
    ...
    % Sexual Reduction
        ...
        % Apply to full subpopulation matrix [Stratsx1]
            vlsReductS = Params.popIndicator * store_vlsReductS;
        
        % Apply to full partner matrix [StratsxStrats]
            Params.inf_vlsReductS_StratsxStrats = repmat(vlsReductS,[1,Params.numStrats]);
...
% ADJUSTED TRANSMISSION RISKS
    ...
    % Vaginal Insertive Risk
        ...
        % Preallocate: apply base risk to full StratsxStratsx Compartments matrix
        BetaFactor.adjPerActVaginalInsProb = inf_basePerActProb_VaginalIns;    
            
        % Apply VLS reduction
            for c = VLSComparts
                BetaFactor.adjPerActVaginalInsProb(:,:,c)= ...
                    inf_basePerActProb_VaginalIns(:,:,c) .*...
                    (1 - Params.inf_vlsReductS_StratsxStrats);
            end
    ...
    % Anal Insertive Risk
        ...
        % Preallocate: apply base risk to full StratsxStratsx Compartments matrix
        BetaFactor.adjPerActAnalInsProb = inf_basePerActProb_AnalIns;
           
        % Apply VLS Reduction
        for c = VLSComparts
           BetaFactor.adjPerActAnalInsProb(:,:,c) = ...     
                inf_basePerActProb_AnalIns(:,:,c) .* ...
                (1 - Params.inf_vlsReductS_StratsxStrats);
        end
...
% COST PER PERSON
    ...
    % Params.hiv_annualCostPerCompart_1(Params.A7), _2, _3 for 3 time periods
...
% INPUTS FOR CALCULATING INTERVENTION COSTS AT DIFFERENT LEVELS IN LAST PERIOD:
% -- % REQUIRING NO INTN (Added by KH on 27Dec2023)
% -- REACH LEVELS BEYOND CALIBRATED CURRENT RATES 
% -- INCREASE IN INTERVENTION COSTS

    % Testing
    Params.intn_Testing_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;

    % Linkage to care at diagnosis
    Params.intn_LTCatDiag_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;
    
    % Linkage to care after diagnosis
    Params.intn_LTCafterDiag_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;
    
    % Initiating ART
    Params.intn_ARTInitiation_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;    
    
    % Treatment Adherence (to become VLS)
    Params.intn_TxAdherenceBecomeVLS_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;

    % Treatment Compliance (to remain VLS)
    Params.intn_TxAdherenceRemainVLS_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;
    
    % Syringe exchange program
    Params.intn_SEP_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;

    % PrEP
    Params.intn_PrEP_NoIntn = ExcelValues_AllParameters(parameter_Index);
    parameter_Index = parameter_Index + 1;
        ...
        switch r
            case 1
                switch HighReachCostIncr
                    case 1
                        Params.intn_Testing_OutreachLvls = importedVariable;
                    case 2
                        Params.intn_LTCatDiag_OutreachLvls = importedVariable;
                    case 3
                        Params.intn_LTCafterDiag_OutreachLvls = importedVariable;
                    case 4
                        Params.intn_ARTInitiation_OutreachLvls = importedVariable;
                    case 5
                        Params.intn_TxAdherenceBecomeVLS_OutreachLvls = importedVariable;
                    case 6 %(Added by KH on 27Dec2023)
                        Params.intn_TxAdherenceRemainVLS_OutreachLvls = importedVariable;
                    case 7
                        Params.intn_SEP_OutreachLvls = importedVariable;
                    case 8
                        Params.intn_PrEP_Oral_OutreachLvls = importedVariable;
                    case 9
                        Params.intn_PrEP_Inject_OutreachLvls = importedVariable;
                end
            case 2
                switch HighReachCostIncr
                    case 1
                        Params.intn_Testing_AddCostatOutreachLvl = importedVariable;
                    case 2
                        Params.intn_LTCatDiag_AddCostatOutreachLvl = importedVariable;
                    case 3
                        Params.intn_LTCafterDiag_AddCostatOutreachLvl = importedVariable;
                    case 4
                        Params.intn_ARTInitiation_AddCostatOutreachLvl = importedVariable;
                    case 5
                        Params.intn_TxAdherenceBecomeVLS_AddCostatOutreachLvl = importedVariable;
                    case 6 %(Added by KH on 27Dec2023)
                        Params.intn_TxAdherenceRemainVLS_AddCostatOutreachLvl = importedVariable;
                    case 7
                        Params.intn_SEP_AddCostatOutreachLvl = importedVariable;
                    case 8
                        Params.intn_PrEP_Oral_AddCostatOutreachLvl = importedVariable;
                    case 9
                        Params.intn_PrEP_Inject_AddCostatOutreachLvl = importedVariable;
                end
        end
    end
...
% INITIAL POPULATION
    ...
    % Pre-allocate
    store_initPop(Params.E6,Params.numStrats) = 0;
...
%% 16. Calc average per-person costs for intervention levels in last time period for all interventions (added by KH on 3Jan2024)

    % Testing
    % Params.TransitionCostPP_Testing(Subpop, HIVStatusStage,TestCostCategory)
    % indicator for if intervention is SSP (used in function for calculating
    % weighted cost per person)
    ind_SEP = 0;
    
    TestCostPPList = [Params.costPP_TestNegRapid_TestPeriod2; ...
        Params.costPP_TestPosRapid_TestPeriod2; ... 
        Params.costPP_TestNegConv_TestPeriod2; ... 
        Params.costPP_TestPosConv_TestPeriod2];
    
    tt_testRate_1 = [Params.tt_testRateUninfected_rp_1 Params.tt_testRateAcute_rp_1 ...
        Params.tt_testRateLatent_rp_1 ...
        Params.tt_testRateLate_rp_1 Params.tt_testRateAIDS_rp_1];
    
    tt_testRate_5 = [Params.tt_testRateUninfected_rp_5 Params.tt_testRateAcute_rp_5 ...
        Params.tt_testRateLatent_rp_5 ...
        Params.tt_testRateLate_rp_5 Params.tt_testRateAIDS_rp_5];

    Params.TransitionCostPP_Testing_T5(Params.numStrats,1 + Params.numHIVstages,Params.numTestCostCategories) = 0;
    for nTestCostCategory = 1:Params.numTestCostCategories % 1:NegRapid, 2:PosRapid, 3:NegConv, 4:PosConv
        for nTestEligStage = 1:Params.numTestEligStates % 1 for uninfected + HIV stages
            
                Params.TransitionCostPP_Testing_T5(:,nTestEligStage,nTestCostCategory) = CalcTransCostPP( ...
                    Params.intn_Testing_NoIntn, RateToProb(tt_testRate_1(:,nTestEligStage)), ...
                    Params.intn_Testing_OutreachLvls, ...
                    Params.intn_Testing_MaxReach, RateToProb(tt_testRate_5(:,nTestEligStage)), ...
                    TestCostPPList(nTestCostCategory), Params.intn_Testing_AddCostatOutreachLvl, ind_SEP);
                
        end % nHIVStatusStage
    end %nTestCostCategory
    % Linkage to care at diagnosis
    Params.TransitionCostPP_ProbLTCatDiag = CalcTransCostPP( ...
            Params.intn_LTCatDiag_NoIntn, Params.tt_linkageFirst_r_1, ...
            Params.intn_LTCatDiag_OutreachLvls, ...
            Params.intn_LTCatDiag_MaxReach, Params.tt_linkageFirst_r_5, ...
            Params.costPP_LTCFirst, Params.intn_LTCatDiag_AddCostatOutreachLvl, ind_SEP);
         
    % Linkage to care after diagnosis and initiating ART
    
    tt_linkageAfterRate_1 = [Params.tt_linkageAfterRate_Acute_1  ...
        Params.tt_linkageAfterRate_Latent_1 Params.tt_linkageAfterRate_Late_1 ...
        Params.tt_linkageAfterRate_AIDS_1];
    
    tt_linkageAfterRate_5 = [Params.tt_linkageAfterRate_Acute_5 ...
        Params.tt_linkageAfterRate_Latent_5 Params.tt_linkageAfterRate_Late_5 ...
        Params.tt_linkageAfterRate_AIDS_5];
    
    tt_ARTInitRate_1 = [Params.tt_ARTInitRateAcute_1 ...
        Params.tt_ARTInitRateLatent_1 Params.tt_ARTInitRateLate_1 ...
        Params.tt_ARTInitRateAIDS_1];
    
    tt_ARTInitRate_5 = [Params.tt_ARTInitRateAcute_5 ...
        Params.tt_ARTInitRateLatent_5 Params.tt_ARTInitRateLate_5 ...
        Params.tt_ARTInitRateAIDS_5];
    
    Params.TransitionCostPP_LTCafterDiag(Params.numStrats,Params.numHIVstages) = 0;
    Params.TransitionCostPP_ARTInitiation(Params.numStrats,Params.numHIVstages) = 0;
    for nDiseaseStage = 1:Params.numHIVstages
        
        Params.TransitionCostPP_LTCafterDiag(:,nDiseaseStage) = CalcTransCostPP( ...
                Params.intn_LTCafterDiag_NoIntn, RateToProb(tt_linkageAfterRate_1(:,nDiseaseStage)), ...
                Params.intn_LTCafterDiag_OutreachLvls, ...
                Params.intn_LTCafterDiag_MaxReach, RateToProb(tt_linkageAfterRate_5(:,nDiseaseStage)), ...
                Params.costPP_LTCAfter, Params.intn_LTCafterDiag_AddCostatOutreachLvl, ind_SEP);
        
        Params.TransitionCostPP_ARTInitiation(:,nDiseaseStage) = CalcTransCostPP( ...
            Params.intn_ARTInitiation_NoIntn, RateToProb(tt_ARTInitRate_1(:,nDiseaseStage)), ...
            Params.intn_ARTInitiation_OutreachLvls, ...
            Params.intn_ARTInitiation_MaxReach, RateToProb(tt_ARTInitRate_5(:,nDiseaseStage)), ...
            Params.costPP_ARTInitiation, Params.intn_ARTInitiation_AddCostatOutreachLvl, ind_SEP);
        
    end    

    % Treatment Adherence (to become VLS)
    Params.TransitionCostPP_TxAdherenceBecomeVLS = CalcTransCostPP( ...
            Params.intn_TxAdherenceBecomeVLS_NoIntn, RateToProb(Params.tt_ARTInitRateFromANV_1), ...
            Params.intn_TxAdherenceBecomeVLS_OutreachLvls, ...
            Params.intn_TxAdherence_MaxReach, RateToProb(Params.tt_ARTInitRateFromANV_5), ...
            Params.costPP_TxAdherence_BecomeVLS, ...
            Params.intn_TxAdherenceBecomeVLS_AddCostatOutreachLvl, ind_SEP);

    % Treatment Compliance (to remain VLS)
    %   Calculate for remaining VLS rather than drop-out (max reach is
    %   already framed that way), but then switch back to drop-out
    Params.TransitionCostPP_TxAdherenceRemainVLS = CalcTransCostPP( ...
            1-Params.intn_TxAdherenceRemainVLS_NoIntn, 1-RateToProb(Params.tt_dropOutRate_VLSToANV_1), ...
            Params.intn_TxAdherenceRemainVLS_OutreachLvls, ...
            Params.intn_TxAdherence_MaxReach, 1-RateToProb(Params.tt_dropOutRate_VLSToANV_5),...
            Params.costPP_TxAdherence_RemainVLS, ...
            Params.intn_TxAdherenceRemainVLS_AddCostatOutreachLvl, ind_SEP);

    % Syringe services program
    %   Calculate percent served by SSPs just before T5. Assume num eligible is static
        ind_SEP = 1;
        NumServedBySEP_hist = Params.tt_numPWIDServedbySEP_Set1;
        NumServedBySEP_T5 = Params.tt_numPWIDServedbySEP_Set2;
        if Params.tt_SEP_YrSet2Begins < Params.tt_periodFiveStartYear
            NumServedBySEP_hist = Params.tt_numPWIDServedbySEP_Set2;
            NumServedBySEP_T5 = Params.tt_numPWIDServedbySEP_Set3;
        end
        if Params.tt_SEP_YrSet3Begins < Params.tt_periodFiveStartYear
            NumServedBySEP_hist = Params.tt_numPWIDServedbySEP_Set3;
            NumServedBySEP_T5 = Params.tt_numPWIDServedbySEP_Set4;
        end    
        if Params.tt_SEP_YrSet4Begins < Params.tt_periodFiveStartYear
            NumServedBySEP_hist = Params.tt_numPWIDServedbySEP_Set4;
            NumServedBySEP_T5 = Params.tt_numPWIDServedbySEP_Set5;
        end
            % (this next set of code for the condition that
            % Params.tt_SEP_YrSet5Begins < Params.tt_periodFiveStartYear
            % in theory shouldn't ever be used since the
            % last set should always be in T5, but I'm adding it for
            % completeness)
            if Params.tt_SEP_YrSet5Begins < Params.tt_periodFiveStartYear
                NumServedBySEP_hist = Params.tt_numPWIDServedbySEP_Set5;
                NumServedBySEP_T5 = Params.tt_numPWIDServedbySEP_Set5;
            end
        numberElig_SEP = Params.raceIndicator' * ...
            ((sum(Params.initPop(Params.UninfectedComparts,:)))' ...
            .* Params.popIndicator(:,Params.pop_IDU) ...
            * Params.behav_pctActivePWID) ;
%         TransitionCostPP_SEP_Arg2 = ProbToRate(Params.raceIndicator * ...
%             (NumServedBySEP_hist ./ max(sum(numberElig_SEP),1)));
%         TransitionCostPP_SEP_Arg5 = ProbToRate(Params.raceIndicator * ...
%             (NumServedBySEP_T5 ./ max(sum(numberElig_SEP),1)));
         PctReach_SEP_Arg2 = ProbToRate( ...
             (NumServedBySEP_hist ./ max(sum(numberElig_SEP),1)));
         PctReach_SEP_Arg5 = ProbToRate( ...
             (NumServedBySEP_T5 ./ max(sum(numberElig_SEP),1)));
    %   Calculate costs PP for SEP  
    Params.TransitionCostPP_SEP_3x1 = CalcTransCostPP( ...
            Params.intn_SEP_NoIntn, PctReach_SEP_Arg2, ...
            Params.intn_SEP_OutreachLvls, ...
            Params.intn_SEP_MaxReach, PctReach_SEP_Arg5,  ...
            Params.costPP_SyringeExchange, Params.intn_SEP_AddCostatOutreachLvl, ind_SEP);

    % PrEP
    %   Pull percent initiating PrEP just before T5
%         Params.PrEP_LastSetBeforePer5 = 1;
        ind_SEP = 0;
        PrEPInitRate_hist = Params.tt_PrEPInitRateOrPctOnPrEP_Set1;
        PctonInjectPrEP_hist = Params.tt_PctonInjectPrEP_Set1;
        PrEPInitRate_T5 = Params.tt_PrEPInitRateOrPctOnPrEP_Set2;
        PctonInjectPrEP_T5 = Params.tt_PctonInjectPrEP_Set2;
        if Params.tt_PrEPInit_YrSet2Begins < Params.tt_periodFiveStartYear
            PrEPInitRate_hist = Params.tt_PrEPInitRateOrPctOnPrEP_Set2;
            PctonInjectPrEP_hist = Params.tt_PctonInjectPrEP_Set2;
            PrEPInitRate_T5 = Params.tt_PrEPInitRateOrPctOnPrEP_Set3;
            PctonInjectPrEP_T5 = Params.tt_PctonInjectPrEP_Set3;
        end
        if Params.tt_PrEPInit_YrSet3Begins < Params.tt_periodFiveStartYear
            PrEPInitRate_hist = Params.tt_PrEPInitRateOrPctOnPrEP_Set3;
            PctonInjectPrEP_hist = Params.tt_PctonInjectPrEP_Set3;
            PrEPInitRate_T5 = Params.tt_PrEPInitRateOrPctOnPrEP_Set4;
            PctonInjectPrEP_T5 = Params.tt_PctonInjectPrEP_Set4;
        end    
        if Params.tt_PrEPInit_YrSet4Begins < Params.tt_periodFiveStartYear
            PrEPInitRate_hist = Params.tt_PrEPInitRateOrPctOnPrEP_Set4;
            PctonInjectPrEP_hist = Params.tt_PctonInjectPrEP_Set4;
            PrEPInitRate_T5 = Params.tt_PrEPInitRateOrPctOnPrEP_Set5;
            PctonInjectPrEP_T5 = Params.tt_PctonInjectPrEP_Set5;
        end
        if Params.tt_PrEPInit_YrSet5Begins < Params.tt_periodFiveStartYear
            PrEPInitRate_hist = Params.tt_PrEPInitRateOrPctOnPrEP_Set5;
            PctonInjectPrEP_hist = Params.tt_PctonInjectPrEP_Set5;
            PrEPInitRate_T5 = Params.tt_PrEPInitRateOrPctOnPrEP_Set5;
            PctonInjectPrEP_T5 = Params.tt_PctonInjectPrEP_Set5;
        end
    %   Calculate costs for PrEP  
    Params.AnnualCostPP_PrEP_Oral_High = CalcTransCostPP( ...
            Params.intn_PrEP_NoIntn, ...
            RateToProb(PrEPInitRate_hist .* (1 - PctonInjectPrEP_hist) ...
                .* Params.PctHighAdherence_OralPrEP), ...
            Params.intn_PrEP_Oral_OutreachLvls, ...
            Params.intn_PrEP_Oral_MaxReach.* (1 - PctonInjectPrEP_hist) ...
                .* Params.PctHighAdherence_OralPrEP, ...
            PrEPInitRate_T5 .* (1 - PctonInjectPrEP_T5) ...
                .* Params.PctHighAdherence_OralPrEP, ...
            Params.hiv_annualCostPerCompart_5(Params.OralPrEPComparts(1)), ...
            Params.intn_PrEP_Oral_AddCostatOutreachLvl, ind_SEP);
    Params.AnnualCostPP_PrEP_Oral_Low = CalcTransCostPP( ...
            Params.intn_PrEP_NoIntn, ...
            RateToProb(PrEPInitRate_hist .* (1 - PctonInjectPrEP_hist) ...
                .* (1- Params.PctHighAdherence_OralPrEP)), ...
            Params.intn_PrEP_Oral_OutreachLvls, ...
            Params.intn_PrEP_Oral_MaxReach.* (1 - PctonInjectPrEP_hist) ...
                .* (1- Params.PctHighAdherence_OralPrEP), ...
            PrEPInitRate_T5 .* (1 - PctonInjectPrEP_T5) ...
                .* (1- Params.PctHighAdherence_OralPrEP), ...
            Params.hiv_annualCostPerCompart_5(Params.OralPrEPComparts(2)), ...
            Params.intn_PrEP_Oral_AddCostatOutreachLvl, ind_SEP);
    Params.AnnualCostPP_PrEP_Inject_High = CalcTransCostPP( ...
            Params.intn_PrEP_NoIntn, ...
            RateToProb(PrEPInitRate_hist .* PctonInjectPrEP_hist ...
                .* Params.PctHighAdherence_InjectPrEP), ...
            Params.intn_PrEP_Inject_OutreachLvls, ...
            Params.intn_PrEP_Inject_MaxReach .* PctonInjectPrEP_hist ...
                .* Params.PctHighAdherence_InjectPrEP, ...
            PrEPInitRate_T5 .* PctonInjectPrEP_T5 ...
                .* Params.PctHighAdherence_InjectPrEP, ...
            Params.hiv_annualCostPerCompart_5(Params.InjectPrEPComparts(1)), ...
            Params.intn_PrEP_Inject_AddCostatOutreachLvl, ind_SEP);
    Params.AnnualCostPP_PrEP_Inject_Low = CalcTransCostPP( ...
            Params.intn_PrEP_NoIntn, ...
            RateToProb(PrEPInitRate_hist .* PctonInjectPrEP_hist ...
                .* (1 - Params.PctHighAdherence_InjectPrEP)), ...
            Params.intn_PrEP_Inject_OutreachLvls, ...
            Params.intn_PrEP_Inject_MaxReach , ...
            PrEPInitRate_T5 .* PctonInjectPrEP_T5...
                .* (1 - Params.PctHighAdherence_InjectPrEP), ...
            Params.hiv_annualCostPerCompart_5(Params.InjectPrEPComparts(2)), ...
            Params.intn_PrEP_Inject_AddCostatOutreachLvl, ind_SEP);

    
    
%% 17. Fxn to calc average cost per transition by time period

function TransitionCostPP = ...
            CalcTransCostPP(InPctNoIntn, InCalibProbBySubpop, ...
            InPctOutreach, ...
            InPctMaxReach, InProb, ...
            InCostStdIntn, InAddCostOutreach, InSEPIndicator)

        % Create matrix of rates defining upper thresholds at which intervention 
        % levels change. Rows are by subpopulation. Columns are 
        % No-intn / Std-intn / Low-outreach / Medium-outreach
        % (Intensive-outreach is everything beyond medium-outreach)
        if InSEPIndicator == 1
            ProbsDefiningIntnLvls(1:Params.numRace, 1:Params.numIntnCostLvls)=0;
        else            
            ProbsDefiningIntnLvls(1:Params.numStrats, 1:Params.numIntnCostLvls)=0;
        end
        % No-intn
        ProbsDefiningIntnLvls(:,1) = InPctNoIntn;
        % Std-intn
        ProbsDefiningIntnLvls(:,2) = max(InCalibProbBySubpop, ...
            ProbsDefiningIntnLvls(:,1));
        % Low-outreach 
        ProbsDefiningIntnLvls(:,3) = ...
            InCalibProbBySubpop+(max(InPctMaxReach-InCalibProbBySubpop,0))*InPctOutreach(1);
        % Medium-outreach
        ProbsDefiningIntnLvls(:,4) = ...
            InCalibProbBySubpop+(max(InPctMaxReach-InCalibProbBySubpop,0))*(InPctOutreach(1) + InPctOutreach(2));
%         % Intensive-outreach (max reach)
%         OutProbsDefiningIntnLvls(:,5) = ProbToRate(0.999);

        % Calculate average cost per person in last time period
        % by summing the percentage of people successfully transitioning getting each
        % intervention level times the incremental cost of that level, then
        % summing over all levels and normalizing to be per person getting the
        % intervention
        TransitionCostPP = ...
            ((max(InProb,ProbsDefiningIntnLvls(:,1))-ProbsDefiningIntnLvls(:,1)) .* InCostStdIntn + ...
            (max(InProb,ProbsDefiningIntnLvls(:,2))-ProbsDefiningIntnLvls(:,2)) .* InAddCostOutreach(1) + ...
            (max(InProb,ProbsDefiningIntnLvls(:,3))-ProbsDefiningIntnLvls(:,3)) .* ...
                (InAddCostOutreach(2) - InAddCostOutreach(1)) + ...
            (max(InProb,ProbsDefiningIntnLvls(:,4))-ProbsDefiningIntnLvls(:,4)) .* ...
                (InAddCostOutreach(3) - InAddCostOutreach(2))) ./ ...
            max(InProb,0.0000001);
