function [Outcome,Params] = HIVEpiModel(ExcelFileName)
%     tic
...
%% 1. Import inputs and set up key runtime variables   
    ...
    [Params] = InitializeParameters(ExcelFileName);
...
%% 2. Code specific to model run type
    ...
    switch Epi1CalLHS2CalOpt3RAOpt4SA5EE6UA7
        ...
        case 1 
            ...
            Outcome = PickOutcomeList(InitOutcome, Params.OutcomesToCollect, ExcelFileName);
        ...
        case 5  
            ...
            % 2.v.2 Set key outcome
                ...
                if Params.outcomeNumber == 1 % QALYS
                    Outcome = AllOutcomes.total_discQALYs;
                ...
                elseif Params.outcomeNumber == 4 
                    Outcome = strcat(num2str(AllOutcomes.total_discQALYs),'-', num2str(AllOutcomes.total_ARTCarePrEPTransitionAndSEPCost_Disc));
...
%             end
        ...
        % 4.2 Pull objective function value from epi model outcomes
            ...
            elseif Params.optObjNum == 3 % Maximize QALYs (= minimize negative QALYs)
                ObjValue_RA = -sum(EpiOutcomesForRA.total_discQALYs,2) / Million
...
%% 9. PickOutcomeList function: Pull outcomes based on the outcome list selected by the user in the Model Settings sheet
    ...
    function [SelectedOutcomes] = PickOutcomeList(AllOutcomes, ListChoice, ExcelFileName)
        ...
        switch ListChoice
            ...
            case 2 %Key outcomes only
                ...
                SelectedOutcomes.ann_discQALYs = sum(AllOutcomes.ann_discQALYs,2);
                ...
                SelectedOutcomes.ann_ARTCarePrEPTransitionAndSEPCost_Disc = sum(AllOutcomes.ann_ARTCarePrEPTransitionAndSEPCost_Disc,2);
            ...
            case 4 % Resource allocation outcomes  
                ...
                MaxReach.PrEP_Oral_HET = max(Params.intn_PrEP_Oral_HET_MaxReach);
                MaxReach.PrEP_Oral_MSM = max(Params.intn_PrEP_Oral_MSM_MaxReach);
                MaxReach.PrEP_Oral_IDU = max(Params.intn_PrEP_Oral_IDU_MaxReach);
                MaxReach.PrEP_Inject_HET = max(Params.intn_PrEP_Inject_HET_MaxReach);
                MaxReach.PrEP_Inject_MSM = max(Params.intn_PrEP_Inject_MSM_MaxReach);
                MaxReach.PrEP_Inject_IDU = max(Params.intn_PrEP_Inject_IDU_MaxReach);
                ...
                SumOutcomes.sum_discQALYsHET_inM = sum(AllOutcomes.ann_discQALYs_byTransGp(:,Params.pop_HET)) / Million;
                SumOutcomes.sum_discQALYsMSM_inM = sum(AllOutcomes.ann_discQALYs_byTransGp(:,Params.pop_MSM)) / Million;
                SumOutcomes.sum_discQALYsIDU_inM = sum(AllOutcomes.ann_discQALYs_byTransGp(:,Params.pop_IDU)) / Million;
                AnnOutcomes.ann_discQALYs_inM = sum(AllOutcomes.ann_discQALYs,2) / Million;
                SumOutcomes.sum_discQALYs_inM = sum(AnnOutcomes.ann_discQALYs_inM);
            ...
            case 6 %Testing frequency outcomes
                ...
                SelectedOutcomes.ann_discQALYs_HET = AllOutcomes.ann_discQALYs_HET;
                ...
                SelectedOutcomes.ann_discQALYs = AllOutcomes.ann_discQALYs;
                SelectedOutcomes.ann_CostTestAndNotify = AllOutcomes.ann_TransCost_CostTestAndNotify_Disc;
                SelectedOutcomes.ann_ARTCarePrEPCost_Disc = AllOutcomes.ann_ARTCarePrEPCost_Disc;
                ...
                % QALYs
                SelectedOutcomes.TotaldiscQALYs_LowRiskHETs = sum(sum(AllOutcomes.ann_discQALYs_HET).* Params.LowRiskHETIndicator');
                SelectedOutcomes.TotaldiscQALYs_HighRiskHETs = sum(sum(AllOutcomes.ann_discQALYs_HET) .* Params.HRHIndicator');
