...
% TransRates is a Compartments x Compartments x168 matrix of transition rates between all
...
%   timestep.
% Rows = source states: number compartments based on disease status and 
%   continuum of care status + NormDeath + AIDSDeath + Aged Out
% Columns = destination states: same as rows
% Layers = stratifications: 168 different stratifications based on 
%   3 risk populations
...
if (Year-floor(Year)) == 0
    ...
    if Year < Params.tt_YrTestSensNotifChange % Assumed not to be in T5
        % % rapid
        TTProg.PctTestsRapid = Params.tt_pctRapid_r_TestPeriod1;
        pctRapid = TTProg.PctTestsRapid;

        % Cost per test 
        costPP_TestNegRapid(1:Params.numStrats,1:Params.numTestEligStates) = ...
            Params.costPP_TestNegRapid_TestPeriod1;
        costPP_TestPosRapid(1:Params.numStrats,1:Params.numTestEligStates) = ...
            Params.costPP_TestPosRapid_TestPeriod1;
        costPP_TestNegConv(1:Params.numStrats,1:Params.numTestEligStates) = ...
            Params.costPP_TestNegConv_TestPeriod1;
        costPP_TestPosConv(1:Params.numStrats,1:Params.numTestEligStates) = ...
            Params.costPP_TestPosConv_TestPeriod1;
       ...
       % Test sensitivity
        ...
        if Year < Params.tt_periodFiveStartYear || Params.tt_progressionSource == 3
            costPP_TestNegRapid(1:Params.numStrats,1:Params.numTestEligStates) = ...
                Params.costPP_TestNegRapid_TestPeriod2;
            costPP_TestPosRapid(1:Params.numStrats,1:Params.numTestEligStates) = ...
        ...
        else %(:,nTestEligStage,nTestCostCategory)
            costPP_TestNegRapid = Params.TransitionCostPP_Testing_T5(:,:,1);
            costPP_TestPosRapid = Params.TransitionCostPP_Testing_T5(:,:,2);
            costPP_TestNegConv = Params.TransitionCostPP_Testing_T5(:,:,3);
            costPP_TestPosConv = Params.TransitionCostPP_Testing_T5(:,:,4);
        end
    ...
    for sectionPeriodSpecific = 1:1
        ...
        if Year < Params.tt_periodTwoStartYear 
            ...
            % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                ...
                TestRate(:,3) =  Params.tt_testRateLatent_rp_1;
                TestRate(:,4) = Params.tt_testRateLate_rp_1;
                TestRate(:,5) =  Params.tt_testRateAIDS_rp_1;
            ...
            % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                ...
                LinkageAfterRate(:,2) = Params.tt_linkageAfterRate_Latent_1; 
                LinkageAfterRate(:,3) = Params.tt_linkageAfterRate_Late_1;
                LinkageAfterRate(:,4) = Params.tt_linkageAfterRate_AIDS_1;
         
            % Drop-out 
            % Updated by LX to include row 6 LAI ART
                ...
                DropOutRate_LAIVLSToLTC = Params.tt_dropOutRate_LAIVLSToLTC_1;
         

            % ART initiation from LTC, no ART effects
            % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                InitiateARTRate(:,1) = Params.tt_ARTInitRateAcute_1; 
                InitiateARTRate(:,2) = Params.tt_ARTInitRateLatent_1; 
                InitiateARTRate(:,3) = Params.tt_ARTInitRateLate_1;
                InitiateARTRate(:,4) = Params.tt_ARTInitRateAIDS_1;

            % ART initiation pct on LAI ART, - Updated by LX to include row 6
                PctInitLAI = Params.tt_pctInitLAI_1;

            % ART initiation from ANV
                ANVToVLSRate = Params.tt_ARTInitRateFromANV_1;

            % Switching, - Updated by LX to include row 6
                % Oral ANV - LAI VLS
                OralANVtoLAIRate = Params.tt_OralANVtoLAI_1;
                % Oral VLS - LAI VLS
                OralVLStoLAIRate = Params.tt_OralVLStoLAI_1;
                % LAI VLS - Oral VLS
                LAItoOralVLSRate = Params.tt_LAItoOralVLS_1;
        ...
        elseif Year < Params.tt_periodFiveStartYear
            ...
            % Testing (with COVID adjustments)
                ...
                TestRate(:,3) =  Params.tt_testRateLatent_rp_2to4 .* (1 - Params.COVID_percentreduc_testrate_PWH(:,COVIDperiod));
                TestRate(:,4) = Params.tt_testRateLate_rp_2to4 .* (1 - Params.COVID_percentreduc_testrate_PWH(:,COVIDperiod));
                TestRate(:,5) =  Params.tt_testRateAIDS_rp_2to4 .* (1 - Params.COVID_percentreduc_testrate_PWH(:,COVIDperiod));
            ...
            % Linkage
                ...
                LinkageAfterRate(:,2) = Params.tt_linkageAfterRate_Latent_2to4; 
                LinkageAfterRate(:,3) = Params.tt_linkageAfterRate_Late_2to4;
                LinkageAfterRate(:,4) = Params.tt_linkageAfterRate_AIDS_2to4;

            % Drop-out, - Updated by LX to include row 6
                ...
                DropOutRate_LAIVLSToLTC = Params.tt_dropOutRate_LAIVLSToLTC_2to4;
            ...
            % ART initiation from LTC, no ART effects
                ...
                InitiateARTRate(:,2) = Params.tt_ARTInitRateLatent_2to4; 
                InitiateARTRate(:,3) = Params.tt_ARTInitRateLate_2to4;
                InitiateARTRate(:,4) = Params.tt_ARTInitRateAIDS_2to4;

            % ART initiation pct on LAI ART, - Updated by LX to include row 6
                PctInitLAI = Params.tt_pctInitLAI_2to4;
            ...
            % Switching, - Updated by LX to include row 6
                % Oral ANV - LAI VLS
                OralANVtoLAIRate = Params.tt_OralANVtoLAI_2to4;
                % Oral VLS - LAI VLS
                OralVLStoLAIRate = Params.tt_OralVLStoLAI_2to4;
                % LAI VLS - Oral VLS
                LAItoOralVLSRate = Params.tt_LAItoOralVLS_2to4;
...
%                     intn_PrEP_IDU_Investment = Params.intn_PrEP_IDU_Investment;
               ...
               for sectionOthProgression = 1:1
                   ...
                   Comparts_CCStage6 = Params.LAIARTVLSComparts;
                    ...
                    DropOutRate_LAIVLSToLTC = Params.tt_dropOutRate_LAIVLSToLTC_5;

                   % Rate of linkage to Care After Diagnosis                
                    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                    LinkageAfterRate(:,1) = Params.tt_linkageAfterRate_Acute_5;
                    LinkageAfterRate(:,2) = Params.tt_linkageAfterRate_Latent_5; 
                    LinkageAfterRate(:,3) = Params.tt_linkageAfterRate_Late_5;
                    LinkageAfterRate(:,4) = Params.tt_linkageAfterRate_AIDS_5;
                    ...
                    % ART initiation pct on LAI ART - Updated by LX to include row 6
                    PctInitLAI = Params.tt_pctInitLAI_5;

                    % Oral ANV - LAI VLS
                    OralANVtoLAIRate = Params.tt_OralANVtoLAI_5;

                    % Oral VLS - LAI VLS
                    OralVLStoLAIRate = Params.tt_OralVLStoLAI_5;
                    
                    % LAI VLS - Oral VLS
                    LAItoOralVLSRate = Params.tt_LAItoOralVLS_5;
               ...
               for sectionIntnTest = 1:1
                   ...
                   % Define baseline variables 
                    ...
                    baseTestRate(:,3) = Params.tt_testRateLatent_rp_5;
                    baseTestRate(:,4) = Params.tt_testRateLate_rp_5;
                    baseTestRate(:,5) = Params.tt_testRateAIDS_rp_5;
                    ...
                    T1TestRate(:,3) =  Params.tt_testRateLatent_rp_1;
                    T1TestRate(:,4) = Params.tt_testRateLate_rp_1;
                    T1TestRate(:,5) =  Params.tt_testRateAIDS_rp_1;
                    ...
                    for TestEligStateNum = 1:Params.numTestEligStates
                        % Next 4 lines added by KH on 24Jan2024. Also
                        % incorporated []_thisState variables into code 
                        costPP_TestNegRapid_thisState = costPP_TestNegRapid(:,TestEligStateNum);
                        costPP_TestPosRapid_thisState = costPP_TestPosRapid(:,TestEligStateNum);
                        costPP_TestNegConv_thisState = costPP_TestNegConv(:,TestEligStateNum);
                        costPP_TestPosConv_thisState = costPP_TestPosConv(:,TestEligStateNum);
                        if TestEligStateNum == 2 % Acute
                            TestSens_Rapid_thisState = TestSens_Rapid_Acute;
                            TestSens_Conv_thisState = TestSens_Conv_Acute;
                            TestSens_Confirm_thisState = TestSens_Confirm_Acute;
                        else
                            TestSens_Rapid_thisState = TestSens_Rapid_Chronic;
                            TestSens_Conv_thisState = TestSens_Conv_Chronic;
                        ...
                        switch TestEligStateNum
                            case 1 % Cost per uninfected person tested (apply to numUninf that are tested)
                                costPP_Test(:,TestEligStateNum) = ...
                                    pctRapid .* (costPP_TestNegRapid_thisState + Params.costPP_TestOutreach'+costPP_NotifyNegRapid) ...
                                        + (1 - pctRapid) .* (costPP_TestNegConv_thisState + Params.costPP_TestOutreach' + costPP_NotifyNegConv);
                            otherwise % Cost per unaware PWH tested (apply to numUnaware that are tested)
                                costPP_Test(:,TestEligStateNum) = ...
                                    (pctRapid * TestSens_Rapid_thisState * (1 - TestSens_Confirm_thisState) .*  ...
                                        (costPP_TestPosRapid_thisState + Params.costPP_TestOutreach' + Params.costPP_NATTest + NotifyRapidProb*costPP_NotifyPosRapid)) + ...
                                    (pctRapid * TestSens_Rapid_thisState * TestSens_Confirm_thisState .*  ...
                                        (costPP_TestPosRapid_thisState + Params.costPP_TestOutreach' + NotifyRapidProb*costPP_NotifyPosRapid)) + ...
                                    ((1 - pctRapid) * TestSens_Conv_thisState * (1 - TestSens_Confirm_thisState) .* ...
                                        (costPP_TestPosConv_thisState + Params.costPP_TestOutreach' + Params.costPP_NATTest + NotifyConvProb*costPP_NotifyPosConv)) + ...
                                    ((1 - pctRapid) * TestSens_Conv_thisState * TestSens_Confirm_thisState .* ...
                                        (costPP_TestPosConv_thisState + Params.costPP_TestOutreach' + NotifyConvProb*costPP_NotifyPosConv)) + ...
                                    (pctRapid * (1-TestSens_Rapid_thisState)  .*  ...
                                        (costPP_TestNegRapid_thisState + Params.costPP_TestOutreach' + NotifyRapidProb*costPP_NotifyNegRapid)) + ...
                                    ((1 - pctRapid) * (1-TestSens_Conv_thisState)  .* ...
                                        (costPP_TestNegConv_thisState + Params.costPP_TestOutreach' + NotifyConvProb*costPP_NotifyNegConv));
                        end
                    end
                    ...
                    % (T1, R1, R2, R3) - result = Strats x 4

                    NumGetIntnAtT1andReachLvlRate_Test(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numTestEligStates)=0;
                    T1andReachLvlRate_Test(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numTestEligStates)=0;
                    CostT1andReachLvlRate_Test(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numTestEligStates)=0;
                    ...
                    for TestEligStateNum = 1:Params.numTestEligStates
                        ...
                        NumGetIntnAtT1andReachLvlRate_Test(:,:,TestEligStateNum) = ...
                            CalcNumIntnAtT1andReachLvlRates(numEligTest(:,TestEligStateNum),  ...
                                T1TestRate(:,TestEligStateNum), Params.intn_Testing_OutreachLvls, Params.intn_Testing_MaxReach, Indicators_LimitPops, InputType_Test);
                        ...
                        CostT1andReachLvlRate_Test(:,:,TestEligStateNum) = ...
                            CalcCostT1andReachLvl(costPP_Test(:,TestEligStateNum), Params.intn_Testing_AddCostatOutreachLvl, ...
                                numEligTest(:,TestEligStateNum), ...
                    ...
                    PctT1andReachLvlRatesAffordedByAlloc_Test(1:Params.numStrats,1:Params.numIntnCostLvls) = 0;
                    ...
                    for TestEligStateNum = 1:Params.numTestEligStates
                         TestRate(:,TestEligStateNum) = baseTestRate(:,TestEligStateNum) + ...
                            ...
                            (T1andReachLvlRate_Test(:,:,TestEligStateNum) - ...
                            [zeros(Params.numStrats,1) T1andReachLvlRate_Test(:,1:Params.numIntnCostLvls-1,TestEligStateNum)]),2);
                    end
                    ...
                    if and(Params.TargetIntnsToYMSM == 1, Params.tt_progressionSource == 3)
                        ...
                        TestRate(:,3) = TestRate(:,3) + Params.tt_testRateLatent_rp_5_nonYMSM;
                        TestRate(:,4) = TestRate(:,4) + Params.tt_testRateLate_rp_5_nonYMSM;
                        TestRate(:,5) = TestRate(:,5) + Params.tt_testRateAIDS_rp_5_nonYMSM;
                    end
               end  
                   ...
                   % Formula:
                    ...
                    numExpectedPosNotified_Rapid = ...
                        pctRapid .* (...
                            ...
                            Compartments(Params.E1,:)' .* TestRate(:,5))).* NotifyRapidProb';
                    numExpectedPosNotified_Conv = ...
                        (1-pctRapid) .* (...
                            ...
                            Compartments(Params.C1,:)' .* TestRate(:,3) + ...
                            Compartments(Params.D1,:)' .* TestRate(:,4) + ...
                            Compartments(Params.E1,:)' .* TestRate(:,5))).* NotifyConvProb'; 
                    ...
                    NumGetIntnAtT1andReachLvlProbs_LTCatDiag = ...
                        CalcNumIntnAtT1andReachLvlRates(numExpectedPosNotifiedTotal, ...
                            Params.tt_linkageFirst_r_1, Params.intn_LTCatDiag_OutreachLvls, Params.intn_LTCatDiag_MaxReach, Indicators_LimitPops, InputType_LTCatDiag);
                    ...
                    CostT1andReachLvlProb_LTCatDiag = ...
                        CalcCostT1andReachLvl(Params.costPP_LTCFirst, Params.intn_LTCatDiag_AddCostatOutreachLvl,...
                    ...
                    LinkageFirstProb = LinkageFirstProb_base + ...
                            ...
                            (T1andReachLvlProb_LTCatDiag - ...
                            [zeros(Params.numStrats,1) T1andReachLvlProb_LTCatDiag(:,1:Params.numIntnCostLvls-1)]),2);
               ...
               for sectionIntnLTCAfter = 1:1
                   ...
                   % Formula:
                    ...
                    LTCAfterEligComparts = [Params.B2, Params.C2, Params.D2, Params.E2];
                   ...
                   % Define baseline variables 
                    ...
                    baseLinkageAfterRate(:,2) = Params.tt_linkageAfterRate_Latent_5;
                    baseLinkageAfterRate(:,3) =  Params.tt_linkageAfterRate_Late_5;
                    baseLinkageAfterRate(:,4) = Params.tt_linkageAfterRate_AIDS_5;

                    % Linkage rates after diagnosis without allocation
                    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                    T1LinkageAfterRate(:,1) = Params.tt_linkageAfterRate_Acute_1;
                    T1LinkageAfterRate(:,2) = Params.tt_linkageAfterRate_Latent_1;
                    T1LinkageAfterRate(:,3) =  Params.tt_linkageAfterRate_Late_1;
                    T1LinkageAfterRate(:,4) = Params.tt_linkageAfterRate_AIDS_1;
                    
                    NumGetIntnAtT1andReachLvlRate_LTCafterDiag(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numHIVstages)=0;
                    T1andReachLvlRate_LTCafterDiag(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numHIVstages)=0;
                    CostT1andReachLvlRate_LTCafterDiag(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numHIVstages)=0;
                    ...
                    for HIVStageNum = 1:Params.numHIVstages
                        ...
                        CostT1andReachLvlRate_LTCafterDiag(:,:,HIVStageNum) = ...
                            CalcCostT1andReachLvl(Params.costPP_LTCAfter, ...
                                Params.intn_LTCafterDiag_AddCostatOutreachLvl, ...
                                numEligLTCAfter(:,HIVStageNum), ...
                    ...
                    for HIVStageNum = 1:Params.numHIVstages
                         LinkageAfterRate(:,HIVStageNum) = baseLinkageAfterRate(:,HIVStageNum) + ...
                            ...
                            (T1andReachLvlRate_LTCafterDiag(:,:,HIVStageNum) - ...
                            [zeros(Params.numStrats,1) T1andReachLvlRate_LTCafterDiag(:,1:Params.numIntnCostLvls-1,HIVStageNum)]),2);
                    end
                    ...
                    if and(Params.TargetIntnsToYMSM == 1, Params.tt_progressionSource == 3)
                        ...
                        LinkageAfterRate(:,2) = LinkageAfterRate(:,2) + Params.tt_linkageAfterRate_Latent_5_nonYMSM;
                        LinkageAfterRate(:,3) = LinkageAfterRate(:,3) + Params.tt_linkageAfterRate_Late_5_nonYMSM;
                        LinkageAfterRate(:,4) = LinkageAfterRate(:,4) + Params.tt_linkageAfterRate_AIDS_5_nonYMSM;
               ...
               for sectionIntnARTInit = 1:1
                ...
                % ART Initiation
                    ...
                    % Number eligible for ART initiation *Updated by LX on
                    % 9/4 to adjust for including LAI
                    ARTInitEligComparts = [Params.B3, Params.C3, Params.D3, Params.E3];
                   ...
                   % Define baseline variables 
                    ...
                    baseARTInitRate(:,2) = Params.tt_ARTInitRateLatent_5;
                    baseARTInitRate(:,3) =  Params.tt_ARTInitRateLate_5;
                    baseARTInitRate(:,4) = Params.tt_ARTInitRateAIDS_5;

                    % ART init rates in T1
                    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                    T1ARTInitRate(:,1) = Params.tt_ARTInitRateAcute_1;
                    T1ARTInitRate(:,2) = Params.tt_ARTInitRateLatent_1;
                    T1ARTInitRate(:,3) =  Params.tt_ARTInitRateLate_1;
                    T1ARTInitRate(:,4) = Params.tt_ARTInitRateAIDS_1;
                                        
                    NumGetIntnAtT1andReachLvlRate_ARTInit(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numHIVstages)=0;
                    T1andReachLvlRate_ARTInit(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numHIVstages)=0;
                    CostT1andReachLvlRate_ARTInit(1:Params.numStrats,1:Params.numIntnCostLvls,1:Params.numHIVstages)=0;
                    ...
                    for HIVStageNum = 1:Params.numHIVstages
                        ...
                        CostT1andReachLvlRate_ARTInit(:,:,HIVStageNum) = ...
                            CalcCostT1andReachLvl(Params.costPP_ARTInitiation, ...
                                Params.intn_ARTInitiation_AddCostatOutreachLvl, ...
                                numEligARTInit(:,HIVStageNum), ...
                    ...
                    for HIVStageNum = 1:Params.numHIVstages
                         InitiateARTRate(:,HIVStageNum) = baseARTInitRate(:,HIVStageNum) + ...
                            ...
                            (T1andReachLvlRate_ARTInit(:,:,HIVStageNum) - ...
                            [zeros(Params.numStrats,1) T1andReachLvlRate_ARTInit(:,1:Params.numIntnCostLvls-1,HIVStageNum)]),2);
                    end
                    ...
                    if and(Params.TargetIntnsToYMSM == 1, Params.tt_progressionSource == 3)
                        ...
                        InitiateARTRate(:,2) = InitiateARTRate(:,2) + Params.tt_ARTInitRateLatent_5_nonYMSM;
                        InitiateARTRate(:,3) = InitiateARTRate(:,3) + Params.tt_ARTInitRateLate_5_nonYMSM;
                        InitiateARTRate(:,4) = InitiateARTRate(:,4) + Params.tt_ARTInitRateAIDS_5_nonYMSM;
               ...
               for sectionARTAdhere_BecomeVLS = 1:1
                    ...
                    NumGetIntnAtT1andReachLvlRates_ARTAdhere_BecomeVLS = ...
                        CalcNumIntnAtT1andReachLvlRates(numberEligARTAdh4to5, ...
                            Params.tt_ARTInitRateFromANV_1, Params.intn_TxAdherenceBecomeVLS_OutreachLvls, Params.intn_TxAdherence_MaxReach, Indicators_LimitPops, InputType_ARTAdhere_BecomeVLS);
                    % Calculate T1 and reach level rates
                    ...
                    CostT1andReachLvlRate_ARTAdhere_BecomeVLS = ...
                        CalcCostT1andReachLvl(Params.costPP_TxAdherence_BecomeVLS, ...
                            Params.intn_TxAdherenceBecomeVLS_AddCostatOutreachLvl, ...
                    ...
                    ANVToVLSRate = ANVToVLSRate_Base + ...
                            ...
                            (T1andReachLvlRates_ARTAdhere_BecomeVLS - ...
                            [zeros(Params.numStrats,1) T1andReachLvlRates_ARTAdhere_BecomeVLS(:,1:Params.numIntnCostLvls-1)]),2);
               ...
               for sectionARTAdhere_RemainVLS = 1:1
                    ...
                    probRemainVLS_OutreachLvls = Params.intn_TxAdherenceRemainVLS_OutreachLvls;
                    ...
                    NumGetIntnAtT1andReachLvlRates_ARTAdhere_RemainVLS = ...
                        CalcNumIntnAtT1andReachLvlRates(numberEligARTAdh5to4, ...
                            probRemainVLS_p1, probRemainVLS_OutreachLvls, Params.intn_TxAdherence_MaxReach, Indicators_LimitPops, InputType_ARTAdhere_RemainVLS);
                    % Calculate T1 and reach level probs
                    ...
                    CostT1andReachLvlRate_ARTAdhere_RemainVLS = ...
                        CalcCostT1andReachLvl(Params.costPP_TxAdherence_RemainVLS, ...
                            Params.intn_TxAdherenceRemainVLS_AddCostatOutreachLvl,...
                    ...
                    PctRemainVLS = pctRemainVLS_Base + ...
                            ...
                            (T1andReachLvlProbs_ARTAdhere_RemainVLS - ...
                            [zeros(Params.numStrats,1) T1andReachLvlProbs_ARTAdhere_RemainVLS(:,1:Params.numIntnCostLvls-1)]),2);
               ...
               for sectionSyringeExchange = 1:1
                   ...
                   PctActivePWIDServedbySEP(1:Params.numRace,1) = 0;
                   TTProg.CostT1andReachLvlPctServed_SEP(1:Params.numRace,1:Params.numIntnCostLvls) = 0;
                   ...
                   for nRace = 1:Params.numRace
                        ...
                        if Params.TargetIntnsToYMSM == 0
                            ...
                            NumGetIntnAtT1andReachLvlPctServed_SEP = ...
                                CalcNumIntnAtT1andReachLvlRates(sum(numberElig_SEP), ...
                                    TTProg.PctOnSEP_T1, Params.intn_SEP_OutreachLvls, Params.intn_SEP_MaxReach, 1, InputType_SEP);
                            ...
                            CostT1andReachLvlPctServed_SEP = ...
                                CalcCostT1andReachLvl(Params.costPP_SyringeExchange, ...
                                    Params.intn_SEP_AddCostatOutreachLvl,...
                            ...
                            PctActivePWIDServedbySEP(nRace,1) = PctActivePWIDServedbySEP_Base + ...
                                    ...
                                    (T1andReachLvlPctServed_SEP - ...
                                    [zeros(1,1) T1andReachLvlPctServed_SEP(:,1:Params.numIntnCostLvls-1)]),2);
                    ...
                    % Apply pct served by race to all Strats subpopulations
                    TTProg.PctActivePWIDServedbySEP = (Params.raceIndicator * PctActivePWIDServedbySEP) .* ...
                    ...
                    for PrEPType = 1:2 % 1 = Oral; 2 = Injectable
                        ...
                        switch PrEPType
                            case 1 % Oral
                                ...
                                intn_PrEP_OutreachLvls = Params.intn_PrEP_Oral_OutreachLvls;
                                intn_PrEP_AddCostatOutreachLvl = Params.intn_PrEP_Oral_AddCostatOutreachLvl; 
                                ...
                                % low adherence
                                AnnualPrEPCostsPP_Initiate = ((Params.PctHighAdherence_OralPrEP * Params.hiv_annualCostPerCompart_5(Params.A7)) ...
                                + ((1 - Params.PctHighAdherence_OralPrEP) * Params.hiv_annualCostPerCompart_5(Params.A8))) .* Indicators_PrEPPops;
                                % Annual PrEP costs per person
                                ...
                                for subpop = 1:Params.numStrats
                                    if sum(Compartments(Params.OralPrEPComparts,subpop))>0
                                        AnnualPrEPCostsPP_OnPrEP(subpop,1) = ((Compartments(Params.A7,subpop) * Params.hiv_annualCostPerCompart_5(Params.A7)) ...
                                        + (Compartments(Params.A8,subpop) * Params.hiv_annualCostPerCompart_5(Params.A8))) ...
                            ...
                            case 2 % Injectable
                                ...
                                intn_PrEP_OutreachLvls = Params.intn_PrEP_Inject_OutreachLvls;
                                intn_PrEP_AddCostatOutreachLvl = Params.intn_PrEP_Inject_AddCostatOutreachLvl; 
                                ...
                                % oral PrEP initiators with high and
                                % low adherence
                                AnnualPrEPCostsPP_Initiate = ((Params.PctHighAdherence_InjectPrEP * Params.hiv_annualCostPerCompart_5(Params.A9)) ...
                                + ((1 - Params.PctHighAdherence_InjectPrEP) * Params.hiv_annualCostPerCompart_5(Params.A10))) .* Indicators_PrEPPops;                                  
                                % Annual PrEP costs per person
                                % currently on PrEP, calculated as a
                                % weighted average based on the number
                                % of people on high adherence and low
                                ...
                                for subpop = 1:Params.numStrats
                                    if sum(Compartments(Params.InjectPrEPComparts,subpop))>0
                                        AnnualPrEPCostsPP_OnPrEP(subpop,1) = ((Compartments(Params.A9,subpop) * Params.hiv_annualCostPerCompart_5(Params.A9)) ...
                                        + (Compartments(Params.A10,subpop) * Params.hiv_annualCostPerCompart_5(Params.A10))) ...
                                        /  sum(Compartments(Params.InjectPrEPComparts,subpop));
    %                                     AnnualPrEPCostsPP_OnPrEP = ((Compartments(Params.A9,:)' * Params.hiv_annualCostPerCompart_5(Params.A9)) ...
    %                                     + (Compartments(Params.A10,:)' * Params.hiv_annualCostPerCompart_5(Params.A10))) ...
    %                                     ./  max(sum(Compartments(Params.InjectPrEPComparts,:),1)',0.000000000000000000001) .* Indicators_PrEPPops;                                   
                        ...
                        NumGetIntnAtT1andReachLvlPctOnPrEP = ...
                            CalcNumIntnAtT1andReachLvlRates(numA1andPrEP_PrEPElig, ...
                                PctonPrEPT1, intn_PrEP_OutreachLvls, intn_PrEP_MaxReach, Indicators_LimitPops, InputType_PrEP); 
                        ...
                        CostT1andReachLvlPctOnPrEP = ...
                            CalcCostT1andReachLvl(AnnualPrEPCostsPP_OnPrEP, ...
                                intn_PrEP_AddCostatOutreachLvl, ...
                        ...
                        PctT1andReachLvlPctOnPrEPAffordedByAlloc_PrEP = ...
                            zeros(Params.numStrats,Params.numIntnCostLvls);

                        %Calculate percentage of T1 % on PrEP and each
                        %reach levels
                        %afforded by allocations to the oral and injectable PrEP interventions

                        TTProg.CostT1andReachLvlRate_PrEP(1:Params.numIntns_PrEPType,1:Params.numPop_plusHETbySex,1:Params.numRace,1:Params.numIntnCostLvls) = 0;
                        ...
                        PctOnPrEP_Among_Elig_ReachableAfforded_Alloc = PctOnPrEP_base + ...
                                ...
                                (T1andReachLvlPctOnPrEP - ...
                                [zeros(Params.numStrats,1) T1andReachLvlPctOnPrEP(:,1:Params.numIntnCostLvls-1)]),2);                        
...
%                                         (NumIntns_Alloc_PrEP(i) - numOnPrEP_byPrEPType(i)) / numA1_PrEPElig(i); 
                        ...
                        switch PrEPType
                            case 1                                 
                                PrEPInitiationRate_A7 = PrEPInitiationRate .* Params.PctHighAdherence_OralPrEP;
                                PrEPInitiationRate_A8 = PrEPInitiationRate .* (1 - Params.PctHighAdherence_OralPrEP);

                                DropOutRate_PrEP_Oral_HighAdherence = DropOutRate_PrEP;
                                DropOutRate_PrEP_Oral_LowAdherence = DropOutRate_PrEP;                                
                            case 2
                                PrEPInitiationRate_A9 = PrEPInitiationRate .* Params.PctHighAdherence_InjectPrEP;
                                PrEPInitiationRate_A10 = PrEPInitiationRate .* (1 - Params.PctHighAdherence_InjectPrEP);
            ...
            else %not allocation-based (direct-entry or HET-testing-frequency in last time period)
              ...
              % Test Rates
                ...
                TestRate(:,3) =  Params.tt_testRateLatent_rp_5;
                TestRate(:,4) = Params.tt_testRateLate_rp_5;
                TestRate(:,5) =  Params.tt_testRateAIDS_rp_5;
                
                LinkageFirstProb = Params.tt_linkageFirst_r_5;

                % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024, - Updated by LX to include row 6
                LinkageAfterRate(:,1) = Params.tt_linkageAfterRate_Acute_5;
                LinkageAfterRate(:,2) = Params.tt_linkageAfterRate_Latent_5; 
                LinkageAfterRate(:,3) = Params.tt_linkageAfterRate_Late_5;
                LinkageAfterRate(:,4) = Params.tt_linkageAfterRate_AIDS_5;
                ...
                InitiateARTRate(:,2) = Params.tt_ARTInitRateLatent_5; 
                InitiateARTRate(:,3) = Params.tt_ARTInitRateLate_5;
                InitiateARTRate(:,4) = Params.tt_ARTInitRateAIDS_5;
                ...
                % Replace HET testing rates with those calc'ed from HET frequency inputs
                    ...
                    TestRate_AIDS_Int = ...
                       (TestRate(:,5) .* (1-Params.popIndicator(:,Params.pop_HET))) + ...
                        ...
                        Params.LowRiskHETIndicator .* (...
                            Params.tt_HETFreq_ApplyLRH * Params.tt_HETFreq_TestRate_LRH + ...
                            (1-Params.tt_HETFreq_ApplyLRH) * TestRate_AIDS_DE);
    ...
    for sectionTPModelStart = 1:1
        ...
        for HIVProgressionSection = 1:1
                ...
                % Calculate rate of HIV progression given years in each stage
                    ...
                    HIVprogRate_ANV_Latent = 1./Params.hiv_durHIVStage_ANV_Latent;
                    HIVprogRate_ANV_Late = 1./Params.hiv_durHIVStage_ANV_Late;
                    
                % Transitions while VLS are determined by rates read from InitParams.m
                % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                 
                    % Initialize index variables
                    Acute = Params.stage_Acute;
                    Latent = Params.stage_Latent;
                    Late = Params.stage_Late;
                    AIDS = Params.stage_AIDS;
                    
                     % Unaware
                     TransRates.ProgressHIV(Params.B1,Params.C1,:) = HIVprogRate_NatHistory(Acute,:);
                     TransRates.ProgressHIV(Params.C1,Params.D1,:) = HIVprogRate_NatHistory(Latent,:);
                     TransRates.ProgressHIV(Params.D1,Params.E1,:) = HIVprogRate_NatHistory(Late,:);
    
                     % Aware
                     TransRates.ProgressHIV(Params.B2,Params.C2,:) = HIVprogRate_NatHistory(Acute,:);
                     TransRates.ProgressHIV(Params.C2,Params.D2,:) = HIVprogRate_NatHistory(Latent,:);
                     TransRates.ProgressHIV(Params.D2,Params.E2,:) = HIVprogRate_NatHistory(Late,:);
    
                     % In Care
                     TransRates.ProgressHIV(Params.B3,Params.C3,:) = HIVprogRate_NatHistory(Acute,:);
                     TransRates.ProgressHIV(Params.C3,Params.D3,:) = HIVprogRate_NatHistory(Latent,:);
                     TransRates.ProgressHIV(Params.D3,Params.E3,:) = HIVprogRate_NatHistory(Late,:);
    
                     % On ART-not-VLS
                     TransRates.ProgressHIV(Params.C4,Params.D4,:) = HIVprogRate_ANV_Latent;
                     TransRates.ProgressHIV(Params.D4,Params.E4,:) = HIVprogRate_ANV_Late;
                     % AIDS/ART-not-VLS: receive the ART death rate
                     
                     % VLS (CD4 count drop)
                     TransRates.ProgressHIV(Params.C5,Params.D5,:) = Params.hiv_rateVLSCD4decr_Latent;
                     TransRates.ProgressHIV(Params.D5,Params.E5,:) = Params.hiv_rateVLSCD4decr_Late;
                     % AIDS/VLS: receive the ART death rate
            
                     % VLS (CD4 count increase)
                     % Latent: no increase
                     TransRates.ProgressHIV(Params.D5,Params.C5,:) = Params.hiv_rateVLSCD4incr_Late;
                     TransRates.ProgressHIV(Params.E5,Params.D5,:) = Params.hiv_rateVLSCD4incr_AIDS;
        end
        ...
        for normDeathSection = 1:1
                ...
                % Do not apply to CC stages
                    % Create matrix where CC compartments are
                        ...
                        % elsewhere
                        indic_ContinuumComparts(Params.CCStages3456,1) = 1;
        ...
        % Death from AIDS if not on ART is determined by inputs for time in
        % each disease stage. 
        % KH moved from .ProgressHIV above to .DieAIDS on 22Jul2024
        for AIDSDeathSection = 1:1
             TransRates.DieAIDS(Params.E1,Params.Compart_AIDSDeath,:) = HIVprogRate_NatHistory(AIDS,:);
             TransRates.DieAIDS(Params.E2,Params.Compart_AIDSDeath,:) = HIVprogRate_NatHistory(AIDS,:);
             TransRates.DieAIDS(Params.E3,Params.Compart_AIDSDeath,:) = HIVprogRate_NatHistory(AIDS,:);
        end
    ...
    for sectionTRYearStart = 1:1
        ...
        for awareSection = 1:1 
            ...
            % tests)
                    ...
                    TTProg.LatentTestRate = TestRate(:,3);
                    TTProg.LateTestRate = TestRate(:,4);
                    TTProg.AIDSTestRate = TestRate(:,5);
        ...
        % 5.iii. Drop out of care, ART-not-VLS, VLS, or PrEP
            ...
            %   Calc: (Pct of people in a stage who drop out of the stage)
               ...
               % Drop off [VLS]
                   ...
                   % [LAI VLS] to [LTC] - updated by LX to include row 6
                    TransRates.DropOut(Params.C6, Params.C3, :) = DropOutRate_LAIVLSToLTC;
                    TransRates.DropOut(Params.D6, Params.D3, :) = DropOutRate_LAIVLSToLTC;
                    TransRates.DropOut(Params.E6, Params.E3, :) = DropOutRate_LAIVLSToLTC;
                
                   % Drop off [PrEP] % JCPrEPUpdate: modified to add dropout
                   % rates for each PrEP state
                    % [PrEP] to [Sus-no-PrEP]
                    TransRates.DropOut(Params.A7, Params.A1, :) = DropOutRate_PrEP_Oral_HighAdherence;
                    TransRates.DropOut(Params.A8, Params.A1, :) = DropOutRate_PrEP_Oral_LowAdherence;
                    TransRates.DropOut(Params.A9, Params.A1, :) = DropOutRate_PrEP_Inject_HighAdherence;
                    TransRates.DropOut(Params.A10, Params.A1, :) = DropOutRate_PrEP_Inject_LowAdherence;
        ...
        % 5.iv. Start ART
            ...
            % Start ART Transitions
                ...
                % Sus-no-PrEP->On-PrEP 
                % All switching inputs:
                    % ANV - LAIVLS
                    % OralVLS - LAIVLS
                    % LAIVLS - OralVLS
                % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                
            % From LTC, no ART effects - updated by LX to include row 6   
                % Eligible: In care, not affected by ART                
                % Note: Acute/In Care flows to Latent/ANV or Latent/VLS

                    % Move to oral ART-not-VLS
                        % Calc: [Rate of ART prescription] * (1 - [Pct become VLS]) * [Pct initiating LAI ART]
                        TransRates.StartART(Params.B3, Params.C4, :) = InitiateARTRate(:,1) * (1-PctWhoBecomeVLS) .* (1-PctInitLAI);
                        TransRates.StartART(Params.C3, Params.C4, :) = InitiateARTRate(:,2) * (1-PctWhoBecomeVLS) .* (1-PctInitLAI);
                        TransRates.StartART(Params.D3, Params.D4, :) = InitiateARTRate(:,3) * (1-PctWhoBecomeVLS) .* (1-PctInitLAI);
                        TransRates.StartART(Params.E3, Params.E4, :) = InitiateARTRate(:,4) * (1-PctWhoBecomeVLS) .* (1-PctInitLAI);

                   % Move to oral ART VLS
                        % Calc: [Rate of ART prescription] * [Pct become VLS] * [Pct initiating LAI ART]
                        TransRates.StartART(Params.B3, Params.C5, :) = InitiateARTRate(:,1) * PctWhoBecomeVLS .* (1-PctInitLAI);
                        TransRates.StartART(Params.C3, Params.C5, :) = InitiateARTRate(:,2) * PctWhoBecomeVLS .* (1-PctInitLAI);
                        TransRates.StartART(Params.D3, Params.D5, :) = InitiateARTRate(:,3) * PctWhoBecomeVLS .* (1-PctInitLAI);
                        TransRates.StartART(Params.E3, Params.E5, :) = InitiateARTRate(:,4) * PctWhoBecomeVLS .* (1-PctInitLAI);

                   % Move to LAI ART
                        % Calc: [Rate of ART prescription] * [Pct initiating LAI ART]
                        TransRates.StartART(Params.B3, Params.C6, :) = InitiateARTRate(:,1) .* PctInitLAI;
                        TransRates.StartART(Params.C3, Params.C6, :) = InitiateARTRate(:,2) .* PctInitLAI;
                        TransRates.StartART(Params.D3, Params.D6, :) = InitiateARTRate(:,3) .* PctInitLAI;
                        TransRates.StartART(Params.E3, Params.E6, :) = InitiateARTRate(:,4) .* PctInitLAI;
            ...
            % From ANV 
                ...
                % [ANV->LAIVLS] - updated by LX to include row 6
                    % Cala: [Rate of ART prescription] * [Pct become VLS] 
                    TransRates.StartART(Params.C4, Params.C6, :) = OralANVtoLAIRate;
                    TransRates.StartART(Params.D4, Params.D6, :) = OralANVtoLAIRate;
                    TransRates.StartART(Params.E4, Params.E6, :) = OralANVtoLAIRate;


            % From Oral ART VLS - updated by LX to include row 6
                % [OralVLS -> LAIVLS]
                    TransRates.StartART(Params.C5, Params.C6, :) = OralVLStoLAIRate;
                    TransRates.StartART(Params.D5, Params.D6, :) = OralVLStoLAIRate;
                    TransRates.StartART(Params.E5, Params.E6, :) = OralVLStoLAIRate;
               

            % From LAI VLS - updated by LX to include row 6
                % [LAIVLS -> OralVLS]
                    TransRates.StartART(Params.C6, Params.C5, :) = LAItoOralVLSRate;
                    TransRates.StartART(Params.D6, Params.D5, :) = LAItoOralVLSRate;
                    TransRates.StartART(Params.E6, Params.E5, :) = LAItoOralVLSRate;    
            ...
            % Start PrEP
                % [Sus-no-PrEP]->[PrEP]
                    if Year < Params.tt_periodFiveStartYear || Params.tt_progressionSource ~= 3 
                        PrEPInitiationRate_A7 = PrEPInitiationRate .* (1 - PctInjectPrEP) .* Params.PctHighAdherence_OralPrEP;
                        PrEPInitiationRate_A8 = PrEPInitiationRate .* (1 - PctInjectPrEP) .* (1 - Params.PctHighAdherence_OralPrEP);
                        PrEPInitiationRate_A9 = PrEPInitiationRate .* PctInjectPrEP .* Params.PctHighAdherence_InjectPrEP;
                        PrEPInitiationRate_A10 = PrEPInitiationRate .* PctInjectPrEP .* (1 - Params.PctHighAdherence_InjectPrEP);
                    ...
                    TransRates.StartART(Params.A1, Params.A10, :) = PrEPInitiationRate_A10;
        ...
        % 5.v. Deaths of individuals with No ART, ANV, and VLS
            ...
            % Death of individuals who are on ART or are VLS
                ...
                % Calc: death rates already set and apply to all Strats strata 
                % ensure dimensionality is right here - (Compartments x Compartments x Strats)
            ...
            % AIDS DEATH
                ...
                % Death from AIDS states not on ART occurs is pulled above
                % at model start since rates don't change over time.
                % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
                
                % ART-not-VLS
                TransRates.DieAIDS(Params.E4,Params.Compart_AIDSDeath,:) = ANVDeathRate_AIDS;
                % VLS
                TransRates.DieAIDS(Params.E5,Params.Compart_AIDSDeath,:) = VLSDeathRate_AIDS;
...
for sectionTimeStepTRs = 1:1
    ...
    if Params.SolveCont ~= 1   
        ...
        % transitions from all PrEP states
            ...
            TransRates.GetInfected(Params.A7,Params.B3,:) = InfRate_PrEP_Oral_High.totalInfRate;
            TransRates.GetInfected(Params.A8,Params.B3,:) = InfRate_PrEP_Oral_Low.totalInfRate;
            TransRates.GetInfected(Params.A9,Params.B3,:) = InfRate_PrEP_Inject_High.totalInfRate;
            TransRates.GetInfected(Params.A10,Params.B3,:) = InfRate_PrEP_Inject_Low.totalInfRate;
        ...
        TransRates.TSRates =  min(real(TransRates.AnnualRates * Params.tt_tstep),1);
...
%% 8a. Calculate number interventions given if applying time period 1 (T1) rates / probs and intervention cost reach threshold rates / probs
    function NumIntnAtT1andReachLvlRates = ...
            CalcNumIntnAtT1andReachLvlRates(nNumEligible, ...
                sngT1RatesProbs, sngOutreachLvls, sngMaxReach, nIndicators_LimitPops, nInputType)
        ...
        if nInputType == 1 % Input is rate (Test, LTC after, ART init, BecomeVLS)
            ...
            sngReachLvl(:,1) = ProbToRate(sngT1Prob + max(sngMaxReach-sngT1Prob,0)*sngOutreachLvls(1)); 
            sngReachLvl(:,2) = ProbToRate(sngT1Prob + max(sngMaxReach-sngT1Prob,0)*(sngOutreachLvls(1)+sngOutreachLvls(2))); 
            sngMaxReach = ProbToRate(sngMaxReach);
        else % Input is % of eligible (LTC at diag, RemainVLS, SEP, PrEP)
            NumIntnPPAtT1Rates = sngT1RatesProbs;
            sngReachLvl(:,1) = sngT1RatesProbs + max(sngMaxReach-sngT1RatesProbs,0)*sngOutreachLvls(1);
            sngReachLvl(:,2) = sngT1RatesProbs + max(sngMaxReach-sngT1RatesProbs,0)*(sngOutreachLvls(1)+sngOutreachLvls(2));
...
%% 8c. Calculate cost to reach time period 1 rates / probs and other 3 reach levels, given base rates get you part way
    function CostT1andReachLvl = ...
            CalcCostT1andReachLvl(sngIntnCostPP, sngAddIntnCostByReachLvl, nNumEligible, sngBaseRatesProbs, ...
        ...
        for ReachLevel = 1:3
            ...
            (sngIntnCostPP_BeyondCurrent + sngAddIntnCostByReachLvl(ReachLevel,1));
...
%% 8d. Calculate percentage of T1 and reach level rates in a subpopulation afforded by each intervention
    ...
    function AddlPctT1andReachLvlRatesAffordedByAlloc = ...
        ...
        if nIndicator_SEP == 1
            AddlPctT1andReachLvlRatesAffordedByAlloc(1,Params.numIntnCostLvls)=0;
        else
            AddlPctT1andReachLvlRatesAffordedByAlloc(Params.numStrats,Params.numIntnCostLvls)=0;
        end
%         for ReachLevel = 1:Params.numIntnCostLvls
%             
...
%                 .* nIndicators_LimitPops;
        ...
        nCostT1andReachLvlRate_acrossEligStages = sum(nCostT1andReachLvlRate(:,:,:),3);
        % Identify reach level thresholds around the reach obtained by the allocation
        if nAlloc >= max(nCostT1andReachLvlRate_acrossEligStages(:,1)'*nIndicators_LimitPops,1)
            if nAlloc >= max(nCostT1andReachLvlRate_acrossEligStages(:,4)'*nIndicators_LimitPops,1)
                nHighestReachLvl = 4;
                nAlloc = ...
                    nCostT1andReachLvlRate_acrossEligStages(:,4)'*nIndicators_LimitPops;
            elseif nAlloc >= max(nCostT1andReachLvlRate_acrossEligStages(:,3)'*nIndicators_LimitPops,1)
                nHighestReachLvl = 4;
            elseif nAlloc >= max(nCostT1andReachLvlRate_acrossEligStages(:,2)'*nIndicators_LimitPops,1)
                nHighestReachLvl = 3;
            else %nAlloc >= max(nCostT1andReachLvlRate_acrossTestEligStages(:,1)'*nIndicators_LimitPops,1)
                nHighestReachLvl = 2;
            end
            nCostPassedReachLvl = nCostT1andReachLvlRate_acrossEligStages(:,nHighestReachLvl-1) .* nIndicators_LimitPops;
            nCostHighestReachLvl = nCostT1andReachLvlRate_acrossEligStages(:,nHighestReachLvl) .* nIndicators_LimitPops;
            ...
            nCostHighestReachLvl = nCostT1andReachLvlRate_acrossEligStages(:,1) .* nIndicators_LimitPops;
