...
%evalDomain=mean(paramHypercube,2)';% ones(1,25)*.5;
load('../paramHypercube_Revised9DecCareful_2.mat');


%load('theta0_pinv13.mat');
%evalDomain=intervalMap_ToUnitInterval(theta0,paramHypercube)';

evalDomain=intervalMap_ToUnitInterval(mean(paramHypercube,2),paramHypercube)';

%load('normBaseVals.mat');
%evalDomain=normBaseVals';
...
normalized_PrincipalDirections=principalDirections*(diag(sqrt(diag(principalDirections'*principalDirections)))\eye(numOutputs,numOutputs));
