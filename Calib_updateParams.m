...
%% 1. Set 2nd Time Period Values to 1st Time Period Values if Not Varying in Calibration
    ...
    if Params.Calib2ndPeriod == 0
        ...
        CalibParams.tt_RelRiskLTC_2to4_D.paramValue(i) = CalibParams.tt_RelRiskLTC_1_D.paramValue(i);
...
%Update the relative risks of being tested
     ...
     % Period 1 
       ...
       % Race/ethnicity
        ...
        % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
            ...
            % CD4>350 (reference case) - not calibrated
            store_tt_relRiskHIVstage_1(Params.stage_Latent,1) = Params.tt_relRiskHIVstage_1(Params.stage_Latent);
            
            % CD4 200-350
            store_tt_relRiskHIVstage_1(Params.stage_Late,1) = CalibParams.tt_relRiskHIVstage_1_D.paramValue(i);
            
            % CD4 < 200
            store_tt_relRiskHIVstage_1(Params.stage_AIDS,1) = CalibParams.tt_relRiskHIVstage_1_E.paramValue(i);
    ...
    %Period 2
        ...
        % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
            ...
            % CD4>350 (reference case) - not calibrated
            store_tt_relRiskHIVstage_2to4(Params.stage_Latent,1) = Params.tt_relRiskHIVstage_2to4(Params.stage_Latent);
            
            % CD4 200-350
            store_tt_relRiskHIVstage_2to4(Params.stage_Late,1) = CalibParams.tt_relRiskHIVstage_2to4_D.paramValue(i);
            
            % CD4 < 200
            store_tt_relRiskHIVstage_2to4(Params.stage_AIDS,1) = CalibParams.tt_relRiskHIVstage_2to4_E.paramValue(i);
...
%Calculate the new annual testing rates using the calibrated relative risks
% Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
...
% Calculate the probability of getting linked to care after 1 year
   ...
   % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
        ...
        Params.store_relRiskLinkageAfter_1(Params.stage_Late) = CalibParams.tt_RelRiskLTC_1_D.paramValue(i);
        Params.store_relRiskLinkageAfter_1(Params.stage_AIDS) = CalibParams.tt_RelRiskLTC_1_E.paramValue(i);
        
        Params.store_relRiskLinkageAfter_2to4(Params.stage_Late) = CalibParams.tt_RelRiskLTC_2to4_D.paramValue(i);
        Params.store_relRiskLinkageAfter_2to4(Params.stage_AIDS) = CalibParams.tt_RelRiskLTC_2to4_E.paramValue(i);
   ...
   % [VLS] -> [ANV] (edited by JC on 11/13/2017)
    ...
    % Update the ART initiation (removed on 2018/01/25 by JC)
        ...
        %rate_ARTInitRate_B_1 = -log(1-CalibParams.tt_ARTInitiation_d_1_B.paramValue(i));
        %rate_ARTInitRate_C_1 = -log(1-CalibParams.tt_ARTInitiation_d_1_C.paramValue(i));
        %rate_ARTInitRate_D_1 = -log(1-CalibParams.tt_ARTInitiation_d_1_D.paramValue(i));
        %rate_ARTInitRate_E_1 = -log(1-CalibParams.tt_ARTInitiation_d_1_E.paramValue(i));
        %rate_ARTInitRate_F_1 = -log(1-CalibParams.tt_ARTInitiation_d_1_F.paramValue(i));
    
        %rate_ARTInitRate_B_2to4 = -log(1-CalibParams.tt_ARTInitiation_d_2to4_B.paramValue(i));
        %rate_ARTInitRate_C_2to4 = -log(1-CalibParams.tt_ARTInitiation_d_2to4_C.paramValue(i));
        %rate_ARTInitRate_D_2to4 = -log(1-CalibParams.tt_ARTInitiation_d_2to4_D.paramValue(i));
        %rate_ARTInitRate_E_2to4 = -log(1-CalibParams.tt_ARTInitiation_d_2to4_E.paramValue(i));
        %rate_ARTInitRate_F_2to4 = -log(1-CalibParams.tt_ARTInitiation_d_2to4_F.paramValue(i));
   ...
   % Update relative risks of ART initiation by race/ethnicity
        ...
        % Expand to full subpopulation matrix [Stratsx1]
   ...
   % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
       ...
       Params.tt_ARTInitRateLatent_1 = Params.rate_ARTInitRate_C_1 * relRiskARTInit_race_1;
       Params.tt_ARTInitRateLate_1 = Params.rate_ARTInitRate_D_1 * relRiskARTInit_race_1;
       Params.tt_ARTInitRateAIDS_1 = Params.rate_ARTInitRate_E_1 * relRiskARTInit_race_1;

       %2nd period
       Params.tt_ARTInitRateAcute_2to4 =  Params.rate_ARTInitRate_B_2to4 * relRiskARTInit_race_2to4;
       Params.tt_ARTInitRateLatent_2to4 = Params.rate_ARTInitRate_C_2to4 * relRiskARTInit_race_2to4;
       Params.tt_ARTInitRateLate_2to4 = Params.rate_ARTInitRate_D_2to4 * relRiskARTInit_race_2to4;
       Params.tt_ARTInitRateAIDS_2to4 = Params.rate_ARTInitRate_E_2to4 * relRiskARTInit_race_2to4;
...
%% 3. Update Disease Progression Parameters
    ...
    % Length of time in ART-not-VLS (modified 08/03/2018)
        ...
        hiv_durHIVStage_ANV_Latent(1,1:4) = CalibParams.hiv_durStage_ANV_Latent_1344.paramValue(i);
        hiv_durHIVStage_ANV_Latent(1,5:Params.numAge-1) = CalibParams.hiv_durStage_ANV_Latent_4564.paramValue(i);
        hiv_durHIVStage_ANV_Latent(1,Params.numAge) = CalibParams.hiv_durStage_ANV_Latent_65.paramValue(i);
...
% DISEASE STAGE TRANSITION RATES WHILE VLS
    ...
    % Decrease in CD4 count 
        ...
        % Apply to parameters
        Params.hiv_rateVLSCD4decr_Latent = CalibParams.hiv_rateVLSCD4decr_Latent.paramValue(i);
...
%% 4. Update the Sexual Mixing Matrix  
    ...
    % TRANSMISSION GROUP / SEX MIXING
        ...
        % Calculate the StratsxStrats matrix for each combination of trans group/sex
        % for sexual mixing
         % KH updated on 20Sep23 to make HET mixing for HRHETs and LRHETs

            % Applying individual factors to a StratsxStrats matrix
    ...
    % RACE MIXING
        ...
        % Apply scalars to a full StratsxStrats matrices - HET    
        ...
        % Apply scalars to a full StratsxStrats matrices - MSM    
...
%% 5. Update the Needle Mixing Matrix
    ...
    % TRANSMISSION GROUP / SEX MIXING        
        ...
        % Combine to one single StratsxStrats matrix
    ...
    Params.behav_NeedleMixing = needleMix_SexRiskType .* ...
...
%% 8. Update the AI Parameters
    ...
    for sectionPctV = 1:1
        ...
        % stratifications)
            ...
            % Populate a [Stratsx1] vector of pct contacts anal
            %    pctContactsAnal = ...
    ...
    for sectionPctInsertive = 1:1
        ...
        %Converting to [Strats x Strats x Compartments]
        for c = 1:Params.numComparts % compartments
    ...
    for sectionMSMPctCondomUse = 1:1
        ...
        % Apply M-F and F-M to [StratsxStrats]
        ...
        % Apply value for MSM-MSM partnerships [StratsxStrats]
        store3_basePctCondomA = storeHET_basePctCondomA + storeMSM_basePctCondomA;

        % Expand to [Strats x Strats x Compartments]
        ...
        % AI Contacts
            ...
            inf_awareComparts_StratsxStratsxComparts(:,:,Params.AwareComparts)=1;
        
            % Apply increase due to awareness
            store_factor_adjPctCondomUseA_NoPrEP_1to4 = behav_basePctCondomA .* ...
                (1 + inf_awareComparts_StratsxStratsxComparts * Params.behav_incrCondomUseDueToDiag_1to4);
            store_factor_adjPctCondomUseA_NoPrEP_5 = behav_basePctCondomA .* ...
                (1 + inf_awareComparts_StratsxStratsxComparts * Params.behav_incrCondomUseDueToDiag_5);
...
%% 9. Update the Factors That Go Into Betas
    ...
    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
        ...
        inf_basePerActProb_VaginalIns(:,:,Params.LatentComparts)=CalibParams.inf_vaginalInsRisk.paramValue(i)  * Params.store_relRiskSexTransmission(Params.stage_Latent);
    ...
    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
        ...
        inf_basePerActProb_AnalIns(:,:,Params.LatentComparts)=CalibParams.inf_analInsRisk.paramValue(i) * Params.store_relRiskSexTransmission(Params.stage_Latent);
    ...
    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
        ...
        inf_basePerActProb_VaginalRec(:,:,Params.LatentComparts)=CalibParams.inf_vaginalRecRisk.paramValue(i) * Params.store_relRiskSexTransmission(Params.stage_Latent);
    ...
    % Updated by KH to collapse CD4350-500 and CD4>500 on 16Jun2024
        ...
        inf_basePerActProb_AnalRec(:,:,Params.LatentComparts)=CalibParams.inf_analRecRisk.paramValue(i) * Params.store_relRiskSexTransmission(Params.stage_Latent);
...
% Update BetaFactors (read into CalcBetas.m)
    ...
    % Vaginal Receptive Risk
        ...
        % Preallocate: apply base risk to full StratsxStratsxCompartments matrix
   ...
   % Anal Receptive Risk
        ...
        % Preallocate: apply base risk to full StratsxStratsx31 matrix
