...
%% 1. Initialize Compartments and Global Variables
    ...
    % Apply ODE-compatible compartment sizes [(Comparts * Strats) x 1] to the 
    % TransRates-compatible "Compartment" variable [Compartments xStrats]
...
%% 5. Calculate number of people flowing between compartments
    ...
    Rates_CompartsxStrats = squeeze(sum((CompartRep.*Rateslong - CompartRepT.*RateslongT),1));
   
    % Calculate new population (based on rates)
    Compartments_AfterRates = Compartments + Rates_CompartsxStrats;
    ...
    delta_pop_CompartsxStrats = Compartments_AfterRatesAndAging - Compartments;
    
    % Convert to ODE-compatible variable size [(CompartsxStrats)x1]
    delta_pop = reshape(delta_pop_CompartsxStrats,1,Params.numComparts*Params.numStrats)';
