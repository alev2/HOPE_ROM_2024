...
%% 1. Calculate number of people aging
    ...
    % numPeopleAging [Compartments xStrats]
...
%% 3. Age people between cohorts (age stratifications)
        ...
        PeopleGoingInto234567(Params.NonAbsorbingComparts, idx_age234567) = ...
            numPeopleAging(Params.NonAbsorbingComparts, idx_age123456);

    % Aging - add people from states (1,2,3,4,5,6) to (2,3,4,5,6,7), subtract from
    % (2,3,4,5,6,7)
        Compartments(Params.NonAbsorbingComparts,idx_age234567) = ...
            Compartments(Params.NonAbsorbingComparts,idx_age234567) ...
            + PeopleGoingInto234567(Params.NonAbsorbingComparts,idx_age234567) ...
            - numPeopleAging(Params.NonAbsorbingComparts,idx_age234567);

        %Subtract from state 1:
        Compartments(Params.NonAbsorbingComparts, idx_age1)= ...
            Compartments(Params.NonAbsorbingComparts, idx_age1) - ...
            numPeopleAging(Params.NonAbsorbingComparts,idx_age1);
...
%% 4. Age into the population
    ...
    % create Compartments xStrats indicators for each trans group / sex / race combo
    counter = 0;
    ...
    % expands back to Compartments xStrats (where only A1 should have non-zeros)
    newPopStrat = sum(sum(newPopStrat,3),1); %first sum dim 3 is 15 combos to 1, then dim 1 is num comparts to 1
    ...
    % sums dim 3 (5 age groups) to get back to Compartments xStrats from Compartments xStratsx5
