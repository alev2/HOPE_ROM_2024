...
% Output:
    % InfRate.lambdaVonly [Stratsx1]
    % InfRate.lambdaAonly [Stratsx1]
    % InfRate.lambdaVsomeAI [Stratsx1]
    % InfRate.lambdaAsomeVI [Stratsx1]
    % InfRate.lambdaN [Stratsx1]
    % InfRate.totalInfRate [Stratsx1]
    % InfRate.RelativeRates [Stratsx5]
...
%% 1. Preallocate local variables
    ...
    % Assign key variables to local variables
        ...
        % Pct of people who have A in M-F partnerships
            ...
            pctPeopleVonly_InMF = 1 - pctPeopleWithA_InMF; % This can be calculated as a complement b/c it's Stratsx1 not StratsxStrats

%% 2. Calculate percent of population infected per stage (compartment)

% Number of people infected in each compartment/subpop
    % [Compartments xStrats]
...
%% 3. Calculate the Number of Infected Partners by Partnership Type
    ...
    % make compart x strats x strats (Compartments xStratsxStrats) version of diag and undiag vector
    ...
    % Expand vector to Compartments xStratsxStrats so it can be multiplied against the
...
%% 4. Calculate the Lambdas (annual probability of an individual being infected)
    ...
    % Formula: 
        ...
        % Overall prob of getting infected [Stratsx1]
    ...
    % Infection from V
        ...
        %  Multiply across compartments and cohorts until [Stratsx1]
        	% Partnership only V
...
%% 5. Calculate the total infection rate and percentage of infections from each other subpop
    ...
    % Calculate percentage of infections from each subpopulation (Strats x
    % Strats) if selected to collect infections by source (on 'Model Settings'
    % sheet)
    ...
    function PctInfSourceBySubpop = CalcPctInfSourceBySubpop(store3variable)
        ...
        % Percentage of infections from each subpopulation (Strats x Strats)
        PctInfSourceBySubpop(Params.numStrats, Params.numStrats)=0;
